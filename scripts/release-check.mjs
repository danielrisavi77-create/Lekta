...
import { TIERS } from './release-tiers.mjs';

const ROOT = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '..');
const OUT = path.join(ROOT, 'docs', 'generated', 'RELEASE_PROOF.json');

// POPIS RAZINA JE PRESELJEN u `scripts/release-tiers.mjs`, doslovno, s komentarima.
//
// Razlog nije urednost nego to sto ga mora citati JOS JEDNA strana: gate pri deployu potpunost
// dokaza od 2026-09-13 racuna NEOVISNO iz `results[]`, umjesto da polje `complete` uzme na rijec
// (vidi `release-gate-core.mjs`). Dok je popis zivio u ovoj datoteci, uvoz je bio nemoguc: ova se
// skripta pri uvozu odmah izvrsi i na kraju zove `process.exit`.
