// scripts/word-verify/repair-core.mts
//
// Jezgra Word oraclea `repair.mts`, izdvojena da je vitest moze pozvati bez procesa i bez argv.
// `repair.mts` ostaje tanki CLI (citaj ulaz, pozovi ovo, zapisi izlaz, ispisi JSON).
//
// Namjerno koristi ISTE pozive kao repair-docx Edge funkcija, da se ne provjerava neka
// paralelna, ljepsa verzija motora.
import { applyFixers, type ApplyFixersResult, type FixerRequest } from '../../src/repair/apply-fixers.ts';
import { readZip, type ZipEntry } from '../../src/repair/zip-codec.ts';
...
export const REQUESTS: FixerRequest[] = [
 ...
 */
async function assistedRequests(bytes: Uint8Array, name: string): Promise<FixerRequest[]> {
  try {
    // Kopija u ArrayBuffer: File ne prima SharedArrayBuffer pogled (tsc strict).
 ...
 *
 * `integrityFailure` je OBAVEZNO polje (null ili {part, problem}). Kad vrata integriteta odbiju
 * popravak, `applyFixers` vraca ULAZNE bajtove bit-identicno, pa bi Word otvorio ORIGINAL i razina
 * lazno prosla. `src/repair/CLAUDE.md` zato trazi izricito `integrityFailure === null`; bez ovog
 * polja PowerShell strana to nije mogla ni vidjeti.
 */
export function buildRepairReport(input: {
  inPath: string;
  before: ZipEntry[];
  after: ZipEntry[];
  assisted: FixerRequest[];
  result: ApplyFixersResult;
}): Record<string, unknown> {
  const { inPath, before, after, assisted, result } = input;
  ...
  return {
    ulaz: inPath,
    // Odmah iza ulaza, namjerno: odbijen popravak je PAD, ne "nista za popraviti".
    integrityFailure: result.integrityFailure
      ? { part: result.integrityFailure.part, problem: result.integrityFailure.problem }
      : null,
...
/** Cijeli prolaz oraclea nad jednim dokumentom: zahtjevi, popravak, izvjestaj. */
export async function runOracleRepair(
  bytes: Uint8Array,
  inPath: string,
): Promise<{ docxBytes: Uint8Array; report: Record<string, unknown> }> {
  const before = await readZip(bytes);
  const assisted = await assistedRequests(bytes, inPath);
  const result = await applyFixers(bytes, [...REQUESTS, ...assisted]);
  const after = await readZip(result.docxBytes);
  return { docxBytes: result.docxBytes, report: buildRepairReport({ inPath, before, after, assisted, result }) };
}
