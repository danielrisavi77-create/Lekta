...
Netlify CLI je autentificiran na odvojeni site `lekta-staging` (`https://lekta-staging.netlify.app`). Dana 2026-09-24 spremljene su staging varijable samo u kontekst `deploy-preview`: `VITE_LEKTA_ENV=staging`, staging `VITE_LEKTA_SUPABASE_URL`, javni staging publishable ključ u `VITE_LEKTA_SUPABASE_ANON_KEY` i `VITE_LEKTA_STUDENT_PILOT=true`. Vrijednost ključa nije zapisana u repozitorij ni ispisana u izvještaj. Predviđeni Netlify deploy-preview origin za ovaj deploy, `https://student-pilot-20260924--lekta-staging.netlify.app`, zasad nije dopušten u Supabase CORS-u za `profile-rules`; OPTIONS za taj origin vraća HTTP 200 bez `Access-Control-Allow-Origin`, dok stabilni `https://lekta-staging.netlify.app` prolazi. Pokušaj čitanja Supabase tajni za sigurno proširenje `ALLOWED_ORIGIN` vratio je `401 Unauthorized`. Ne prepisivati tajnu napamet; dok staging pristup ne dopusti provjeru/promjenu te vrijednosti, preview ne dokazuje puni browser tok profila. Ne graditi staging pregled bez eksplicitnih staging vrijednosti jer zadani build može usmjeriti aplikaciju na produkciju. Tajne ne zapisivati u repozitorij ni logove.
