...
| `footnote-typography-fixer` | `fontName,fontSizePt,alignJustify,deep`; barem jedno od prva tri polja, font 1-100, veličina 6-72, `deep` boolean |
...
### Target i lokalni output hash

`targetSha256` veže ugovor uz točnu serverski popravljenu DOCX referencu koju runner preuzima i
rekonstruira. `outputSha256` u potpisanom `completed` događaju veže potvrdu uz novu DOCX datoteku
koju je WordReplica stvarno proizvela. Te dvije datoteke nisu binarno iste: Word rekonstrukcija
može promijeniti ZIP zapis, metadata i druge nevidljive bajtove i kada G0-G9 potpuno prolaze.
Server zato sprema oba hasha odvojeno i ne zahtijeva njihovu jednakost.
