# Lekta detailed project rules

Ovo je detaljni source of truth za projektne invarijante koje ne trebaju biti u always-on
agent kontekstu. Root `AGENTS.md` je kratka mapa; multi-provider ugovor je
`docs/agents/ORCHESTRATION.md`.

Citaj samo odjeljke relevantne za zahvat. Povijesni razlozi ostaju ovdje jer objasnjavaju
zasto guard postoji, ali nisu obavezni kontekst svakog zadatka.
...
Kanonski multi-provider routing, billing, context, usage i cross-provider review ugovor zivi u
`docs/agents/ORCHESTRATION.md`. Ovaj detaljni vodic ne duplicira taj ugovor.
