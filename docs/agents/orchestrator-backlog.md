...
| F10 | Z15 mobilni list | Podnozje lista nosi "Aa · Prikaz" | Panel "Prilagodi prikaz" (Z6) zivi SAMO na `/` i `/rad/`, a dva elementa s `id="displayBtn"` su nevaljan HTML | ODLUKA PROVEDENA 2026-09-23: `mountDisplaySettings(document)` je preseljen iz `routes/intake/main.ts` i `routes/workspace/main.ts` u `shared/ui-boot.ts`, JEDNIM pozivom koji pokriva sve rute (u traku ne smije: `tests/display-settings.test.ts` mjeri da je traka ne uvozi, a `tests/route-shell-budget.test.ts` njen graf uz granicu od 8 KB gzip). Panel prima dva otvaraca: `#displayBtn` u traci, koji je i dalje JEDINSTVEN u dokumentu, i `[data-display-open]` u mobilnom listu; posrednik je uklonjen. Oba nose `aria-expanded` i `aria-controls`, oba prate isto stanje, a Esc vraca fokus na onaj koji je panel otvorio (na mobitelu je `#displayBtn` skriven, pa bi fiksno vracanje bilo vracanje u nista). Svih 13 stranica s trakom nosi `#displayBtn` i otvarac u listu | ODLUCENO 2026-09-23: (a) panel "Prilagodi prikaz" (Z6) montira se na SVE rute kroz ui-boot; #displayBtn jedinstven, mobilni list zove isti panel. (bilo: Treba li panel Z6 doci na sve rute (tada posrednik postaje obican gumb), ili kontrola ostaje samo na dvije rute?) |
...
| F13 | Z16 stepper na mobitelu | Z16: "stepper na /rad/ prelama u dva reda" | Dva reda pilula na 390 px dizu zaglavlje preko proracuna (mjereno 177 px), a Z16 je NEPOCET zadatak | ODLUKA PROVEDENA. CSS je bio na mjestu od kruga popravka (natpisi vizualno skriveni `clip-path`-om u `@media (max-width: 819px)`, broj ostaje vidljiv, pa je stepper "01 02 03 04" u JEDNOM redu uz ime dokumenta). DOVRSEN JE GARD 2026-09-23: stara tvrdnja je trazila pravilo BILO GDJE u listu, pa bi ista kopija u osnovnom sloju (natpisi skriveni i na 1180 px, gdje su cijela poanta koraka) prosla jednako. Sada se mjeri da je pravilo UNUTAR mobilnog bloka i da ga u osnovnom sloju NEMA, natpisi i brojevi se usporedjuju s MODELOM (`siteChromeSteps`), a mutacija pokriva oba smjera (preseljeno pravilo, `display: none` umjesto `clip-path`). Obje mutacije su izvrsene nad stvarnim listom i obje obore gard. Visina zaglavlja na 390 px NIJE ponovno mjerena u ovom krugu (Playwright se nije pokretao); vrijedi mjerenje iz kruga popravka, 113 px naspram proracuna od 124 px | ODLUCENO 2026-09-23: (a) na < 820px koraci bez natpisa ("01 02 03 04", natpis u aria-label) do Z16; Z16 odlucuje konacno mjesto. (bilo: Je li "01 02 03 04" bez natpisa prihvatljiv mobilni stepper do Z16, ili Z16 treba odluciti drugo mjesto za korake na uskom zaslonu?) |
