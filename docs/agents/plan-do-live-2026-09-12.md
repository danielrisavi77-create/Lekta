...
- [x] Provjeriti redirect URL-ove, CORS, domene, tajne po okruženju i nemogućnost da staging frontend slučajno koristi produkcijsku naplatu ili storage. Staging frontend je `https://lekta-staging.netlify.app`, bundle odabire staging Supabase ref, `ALLOWED_ORIGIN` reflektira samo taj origin, a extraction probe prolazi. Dashboard-only redirect allow-lista ostaje izvan javnog API dokaza.
- [x] Izvesti DB smoke i migracijske provjere u namjenskoj testnoj bazi. Produkcijski migracijski identitet uspoređen je read-only; drift je dokumentiran, a nikakvo usklađivanje nije pokrenuto bez zasebnog plana.
