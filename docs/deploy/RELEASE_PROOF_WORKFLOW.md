...
|---|---|---|---|
| a | `dist/build-info.json` nedostaje, nije JSON ili nema 40-znamenkasti commit | `verify-deploy-dist`, `verify-release-proof` | `dist/build-info.json ne postoji` |
| b | `dist/build-info.json` nosi commit koji NIJE onaj koji se gradi | isto | `artefakt nema identitet builda` |
| c | dokaz je `stale` ili `unknown` | isto | `ZASTARJELO` / `NE ZNAM` |
...
5. **Commitaj samo datoteku dokaza** (`git commit --only docs/generated/RELEASE_PROOF.json`) i
   ...
   ```powershell
   npm run release:proof-gate          # mek: javlja upozorenje
   $env:LEKTA_REQUIRE_RELEASE_PROOF = "1"; npm run release:proof-gate   # tvrd: "ne znam" je pad
   ```
...
| `npm run release:proof-gate` | SAMO gate dokaza i identiteta artefakta, bez gradnje (korak 5). |
...
- `tests/release-gate-wiring.test.ts` - da lanac objave zove PUNI gate i da cron nadzor ostaje blag.
