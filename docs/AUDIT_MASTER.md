...
| **NOVO, izvan audita** | **Pod stvarnom mobilnom emulacijom (`isMobile` + `hasTouch`) klik na "Nastavi na profil" nije prolazio ni u 120 s.** Na desktop Chromeu isti test prolazi, pa je nalaz bio nevidljiv. **ZATVORENO 1.9.2026.** Uzrok nije bila dodirna meta nego okomiti proračun na niskom zaslonu: `#browseBtn` završava točno na 667, pa je dropzone prelazio pregib za svojih 14 px paddinga i 1 px ruba. Uz to stavka `.wizard-grid` ima zadani `min-width:auto`, pa stupac nije mogao ispod svoje min-content širine (dropzone 378 px na zaslonu od 375, a `body` ima `overflow-x:clip` pa se višak tiho REZAO) | Popravljeno u `index.html` pod `(max-width:720px) and (max-height:720px)`; `roadmap-v2` je izvađen iz `testIgnore` i vrti se zeleno u oba projekta |
| **NOVO, izvan audita** | **Napredna provjera odzumirala je stranicu na ~52 %, pa dodirne mete više nisu pogađale.** `.result-cockpit-advanced-shell` je grid, a stavka grida ima zadani `min-width:auto`: sadržaj je tražio 710 px u spremniku od 353, pa je layout skakao s `393x727` na `752x1392` u trenutku otvaranja nalaza. Klik na "Prilagodi popravke" nije prolazio ni u 120 s, a Playwright je prijavljivao da ga presreće `deadline-card`, što je slalo na krivi trag. Nađeno PRVIM mobilnim prolazom `repair-panel.spec.ts`, koji je dotad tvrdo postavljao 1440x1000 i time davao nula mobilne pokrivenosti plaćenom dijelu proizvoda (UX-02) | `.result-cockpit-advanced-shell > * { min-width: 0 }`; s njim `404x746`. Tvrdi viewport maknut, spec prilagođen toku koji se stvarno razlikuje po širini, `repair-panel` izvađen iz mobilnog `testIgnore`: 4/4 mobile-chromium i 4/4 chromium |
| **NOVO, izvan audita** | **Prva vožnja `mobile-webkit` projekta ikad (iPhone 13).** Kritični put (`mobile-critical-path`) PROLAZI, dakle upload, profil, analiza i rezultat rade i u Safarijevu motoru. Dva nalaza ostaju otvorena: (1) na 390x844 nakon "Nastavi na profil" `data-step` je PRAZAN niz umjesto `"2"`, dakle WebKit se ponaša drukčije od Chromiuma na istom prijelazu; (2) na 375x667 `page.goto` istekne u 120 s | Zapisano, ne popravljeno. Projekt namjerno NIJE u gateu dok nema prvi zeleni prolaz, po vlastitom pravilu matrice. Preostaje i ~10 px vodoravnog prelijevanja u `#analyzer` (404 umjesto 393) |
| **NOVO, izvan audita, OTVOREN, PLACENI PUT** | **Otisak sidra nije isti u pregledniku i u Edge funkciji (T15, prvi korak).** `anchorFingerprintForXml` racuna otisak iz STRUKTURE preko `DOMParser`-a; kad parsiranje ne uspije, `catch` grana vraca posve drugi otisak iz regex normalizacije. Izmjereno IZVODJENJEM na istom ulazu: Deno `typeof DOMParser === "undefined"` daje `56373eeb`, Node/preglednik daje `b7bbf729`. `supabase/functions/deno.json` dodaje `dom` u `lib`, ali to su TIPOVI za `deno check`, ne runtime | Vazno jer je `element-caption-fixer` u serverskom lancu (`apply-fixers.ts:85`) i na retku 275 usporedjuje PONOVNO IZRACUNAT otisak s onim koji stize u zahtjevu, a klijent ga racuna u pregledniku. Dvije strane usporedbe nastaju u razlicitim granama iste funkcije. NE popravljam: dira analitcku jezgru koja je golden-zasticena. Zakljucano karakterizacijskim testom `tests/anchor-fingerprint-environment.test.ts`, koji PADA cim netko funkciju ucini okolinski neovisnom, sto je zeljeni ishod. NIJE provjeren puni put parametara od analize do Edge funkcije i to se ne tvrdi |
| **NOVO, izvan audita, OTVOREN** | **T16 je jedina stavka plana koja je otisla UNATRAG.** `src/ui/app.ts` je narastao s 334 KB / 1977 redaka na **359 KB / 2389**, a broj rucnih dodira `hidden` s 28 na **97**. Plan za tu stavku trazi ratchet PRIJE premjestanja koda; da je postojao, ovog rasta ne bi bilo | Postavljena su dva garda, oba s DONJOM granicom da budzet ne ostane naduvan nakon prvog izdvajanja: `tests/ui-module-budget.test.ts` (app.ts <= 359 KB, `src/ui` <= 821 KB, dodiri `hidden` <= 97) i `tests/module-graph.test.ts`. Drugi je usput izmjerio nesto sto nitko nije pratio: **16 kruznih uvoza** u `src/`, gotovo svi po obrascu `fixers.ts` -> pojedinacni fixer -> `fixers.ts`. Ne popravljaju se usput jer diraju golden-zasticenu repair jezgru; ratchet stoji na zatecenom broju. Sama stavka (razbijanje app.ts) i dalje je OTVORENA, gardovi je samo zaustavljaju da propada |
| **NOVO, izvan audita, CEKA VLASNIKA** | **Deploy manifest je postojao samo kao ideja (T12).** Izvjestaj o driftu usporedjuje repozitorij s deployanim stanjem, ali nije imao OCEKIVANJE s kojim bi ga usporedio, pa "produkcija ima 18 od 22" nije razlikovalo namjeru od propusta | Novi `supabase/deploy-manifest.json` (`npm run deploy-manifest`) nosi 24 funkcije, IZVEDENE iz koda: ime, `verify_jwt` iz `config.toml` i tajne koje funkcija stvarno cita (`Deno.env.get`). Tri polja su ODLUKA a ne cinjenica i namjerno stoje prazna na svih 24: `owner`, `intentionalExclusion`, `reason`. Gard `tests/deploy-manifest.test.ts` (6 testova, uz mutaciju koja izbacuje funkciju) drzi da manifest ne odluta, a ratchet da broj funkcija bez vlasnika smije samo padati. **Plan je i ovdje imao zastarjelu tvrdnju:** tri funkcije bez `verify_jwt` deklaracije vise ne postoje, sve 24 su deklarirane |
| **NOVO, izvan audita, CEKA VLASNIKA** | **Release dokaz je 8/9, i jedina prepreka je vlasnicka, ne tehnicka (T11).** Izmjereno na `docs/generated/RELEASE_PROOF.json`: osam razina prolazi (check, edge, conformance, slow, ux, strict-open, word, word-worst), a deveta, `extraction`, stoji na `unavailable` uz razlog `missing-env:LEKTA_STAGING_ORIGIN`. Kako je oznacena obaveznom, `complete` NIKAD ne moze postati istina, pa bi ukljucivanje `LEKTA_REQUIRE_RELEASE_PROOF=1` blokiralo svaki deploy. Sam dokaz je uz to star 39 commita (pecen 2026-09-01 na `ab2a108`, doduse nad CISTIM stablom) | Plan je za ovu stavku imao dvije zastarjele tvrdnje: dokaz danas ima 9 razina (ne 8), a obje varijable (`LEKTA_STAGING_ORIGIN`, `LEKTA_REQUIRE_RELEASE_PROOF`) SU dokumentirane u `.env.example`. Preostaje odluka koju ne mogu donijeti umjesto vlasnika: ili se dade zivi staging origin (probe ga trazi i izricito odbija produkcijski host), ili se `extraction` oznaci neobaveznom uz imenovan, zaseban zapis da staging dokaz nedostaje. Dok se to ne rijesi, zatezanje gatea je zamka za onoga tko ga ukljuci |
| **NOVO, izvan audita, OTVOREN** | **Nazivnici se razilaze, i razlika nosi dva stvarna kvara (T17).** Registar ima 407 profila, ledger 436 redaka, a jedinstvenih profila u njima 410. Dok to nije imenovano, svaki postotak nad "brojem profila" znaci nesto drugo ovisno o tome tko ga racuna | Novi `docs/generated/STATUS.json` (`npm run status`) imenuje nazivnike i izdvaja dvije anomalije: **3 rute bez profila** (`fpzg-ba-vvu`, `fpzg-joint-se`, `pravo-joint-repic`, student ih moze odabrati a odabir ne vodi nikamo) i **3 profila bez jedinice** (`radno-socijalno-pravo`, `sociologija`, `trgovacko-pravo`, imaju VERIFICIRANA pravila ali nisu dostizni kroz odabir, dakle rade se pravila koja nitko ne moze dobiti). Gard `tests/status-artifact.test.ts` je ratchet: brojevi smiju samo padati. Artefakt je ujedno prvi koji nosi `generatedAt` i `generatedFromCommit`; ostala 21 ih nema |
| **NOVO, izvan audita, ZATVORENO** | Plan je za T17 tvrdio CP1252 dvostruko kodiranje u `docs/VISION.md`, `docs/UX_PRINCIPLES.md` i `MONETIZATION_AND_ANTI_ABUSE.md` | **Provjereno 2026-09-03: nula tragova i nula U+FFFD u sve tri datoteke.** Netko je to sanirao u medjuvremenu; stavka se skida s popisa umjesto da se ponovo "popravlja" |
| **NOVO, izvan audita, OTVOREN** | **Prvo axe mjerenje stvarnih stanja proizvoda ikad (T8).** Zatečeno: axe se vrti u 5 datoteka, ali SVAKI poziv dolazi odmah nakon `goto`, i to na demo, galerijskim i alat rutama. Glavni tok analizatora nije imao nijednu provjeru poslije ijedne interakcije. Novi `tests/ux/a11y-states.spec.ts` obilazi čarobnjak 1 do 3, rezultat, otvorene nalaze, modal pregleda, panel popravka i ledger modal, iz JEDNE analize | **Nađeno 8 pravila / 18 čvorova u TRI stanja**, sva ostala čista: `nalazi otvoreni` i `panel popravka` nose `aria-allowed-attr` (critical, 4x svaki), `aria-required-children` (critical) i `color-contrast`; `ledger modal` nosi `aria-dialog-name` i `list`. Popravak NIJE napravljen: sve je u kokpitu i panelu popravka, površini koju druga sesija aktivno redizajnira, pa je odluka vlasnikova. Gard je ratchet, ne prag: novo kršenje pada odmah, ukupan broj smije samo padati. **Dvoje ostaje neizmjereno i imenovano:** stanje greške pri uploadu (`#dropError`; ulazna vrata su fail-open pa dva pokušaja izvana nisu do njega došla) i mobilni projekt (šetnja pod Pixel 5 emulacijom prijeđe 600 s, pa se promiče tek nakon prvog zelenog prolaza) |
| **NOVO, izvan audita** | **Nativni preglednički `DOMParser` prvi put je stvarno izvršen nad `.docx` XML-om (T10).** Analiza ima dvije izvedbe koje nisu isti kod: worker parsira preko `@xmldom/xmldom`, inline fallback preko nativnog `DOMParser`-a. Razliku dosad nije mjerilo ništa, jer `vitest.config.ts` podmeće xmldom kao globalni `DOMParser` u SVIM testovima, pa je postojeći test "bez Workera vraća identičan rezultat" uspoređivao xmldom protiv xmldoma. Pad na inline granu bio je uz to gol `console.warn`, bez ijednog brojača | `tests/ux/parser-parity.spec.ts` gasi `Worker` prije učitavanja stranice i uspoređuje ocjenu, broj grešaka i skup nalaza. **Na Chromiumu: identično.** Uveden biljeg putanje (`__lektaAnalysisPath`) i brojač padova, pa test tvrdi da su putanje bile RAZLIČITE; bez toga bi prolazio vakuumski. Opseg tvrdnje je jedna fixtura i jedan motor |
| **NOVO, izvan audita, OTVOREN uzrok, ZATVOREN utjecaj** | **U WebKitu se dogadaj `load` ne okine, i to na PRODUKCIJSKOM buildu.** Izmjereno nad `vite preview`: WebKit ne okine `load` ni u 90 s, dok ga Chromium okine za 2,9 s; `domcontentloaded` je uredan u oba (WebKit 3,7 s, Chromium 1,4 s). Isključeno je troje: fontovi (nula odbijenih zahtjeva), dev poslužitelj (mjereno na buildu) i `<video>` element (uklonjen kroz `MutationObserver`, i dalje visi). Resource Timing pritom prijavljuje **nula** nedovršenih resursa, pa uzrok ostaje NEIMENOVAN. Uz to, u WebKitu ni klik koji pokreće analizu (`#stepToAnalyze`) ne dovrši u 600 s, pa parnost ondje OSTAJE NEIZMJERENA; provjereno je da uzrok NIJE nepodržan modularni Worker (oba motora ga konstruiraju i dobivaju odgovor). Najvjerojatnije je riječ o brzini: dva trivijalna testa u WebKitu ovdje traju 4 minute | **Korisnika ne dira, i to je izmjereno, ne pretpostavljeno:** u cijelom repozitoriju nema nijedne pojave `window.onload` ni `addEventListener('load')`, a boot ide na `DOMContentLoaded` (5 modula). Šteta je ograničena na ALAT: Playwrightov zadani `waitUntil: 'load'` visi, što objašnjava i raniji istek `page.goto` u `mobile-webkit` projektu. `parser-parity.spec.ts` zato koristi `domcontentloaded`, uz bilješku. Raniji zapis ovdje je tvrdio da "neki podresurs nikad ne dovrši učitavanje"; to je POVUČENO, jer Resource Timing pokazuje suprotno | **DOPUNA 2026-09-03: parnost je izmjerena i u WebKitu i u Firefoxu, i DRZI.** Na mirnom stroju (1361 MB slobodno, 1 node proces) WebKit prolazi u 2,7 i 2,3 min u dva uzastopna prolaza, Firefox u 1,6 min; ocjena, broj gresaka i skup nalaza identicni su worker putanji. Raniji istek od 600 s bio je OPTERECENJE STROJA (25 tudjih node procesa), ne kvar: ista provjera uz jedan proces prolazi. Nalaz o `load` dogadaju ostaje, jer je neovisan i reproducibilan; `parser-parity` je promaknut u matricu za firefox i webkit.
