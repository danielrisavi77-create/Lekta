...
| **NOVO, izvan audita, OTVORENO** | **WebKit: `page.goto` sa zadanim `waitUntil: 'load'` ne završi ni u 600 s**, dva puta zaredom, dok Chromium isti prolaz odradi u 4,5 min. Znači da neki podresurs u WebKitu nikad ne dovrši učitavanje. S `waitUntil: 'domcontentloaded'` navigacija prolazi, pa je uzrok potvrđen. Nakon toga tok ipak ne stigne do koraka 3 (`#analyzeBtn` se ne pojavi), pa parnost u WebKitu OSTAJE NEIZMJERENA | Zapisano, ne popravljeno, i namjerno se NE tvrdi da je riječ o kvaru u proizvodu: `mobile-critical-path` u WebKitu prolazi, pa uzrok može biti i pretpostavka samog speca (klik na `#uploadCtaBtn`, koji taj drugi spec ne radi). Razdvajanje ta dva objašnjenja je sljedeći korak. `parser-parity` zato NIJE dodan u `testMatch` za firefox/webkit: matrica se promiče tek nakon prvog zelenog prolaza |
