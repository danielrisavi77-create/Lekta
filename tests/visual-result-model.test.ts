...
describe('VisualResultModel', () => {
  ...
  it('keeps unscored results from pretending they are 0 of 100', () => {
    ...
    expect(model.score).toEqual({ kind: 'unscored', label: 'Nije bodovano', reason: 'Rezultat nema bodovanu tehnicku ocjenu.' });
  ...
  it('uses supplied Document DNA segments and otherwise returns a safe unavailable fallback without page invention', () => {
    const withDna = buildVisualResultModel(baseResult, {
      documentDnaSegments: [
        { id: 'font', label: 'Dominantni font', value: 'Inter' },
        { id: 'toc', label: 'Sadrzaj', value: 'pronadjen', page: null },
      ],
    });
    const withoutDna = buildVisualResultModel(baseResult);

    expect(withDna.documentDna).toEqual({
      kind: 'available',
      segments: [
        { id: 'font', label: 'Dominantni font', value: 'Inter' },
        { id: 'toc', label: 'Sadrzaj', value: 'pronadjen', page: null },
    ...
    expect('page' in withDna.documentDna.segments[0]).toBe(false);
    expect(withoutDna.documentDna).toEqual({ kind: 'unavailable', reason: 'Document DNA is not available for this result.' });
  });
});
