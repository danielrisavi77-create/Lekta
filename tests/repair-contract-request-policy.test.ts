...
const assistedSamples = [
  ...
  ['required-section-fixer', 'sections', {
    version: 1,
    sections: [{ id: 'abstract-1', kind: 'abstract', label: 'Abstract', insertionAnchor: { paragraphIndex: 2, anchorFingerprint: 'paragraph-a1', anchorText: 'Uvod', position: 'before' }, headingLevel: 1, numbered: false, confirmed: true }],
  }],
  ['link-doi-fixer', 'operations', {
    version: 1,
    operations: [{ id: 'link-1', part: 'word/document.xml', paragraphIndex: 2, start: 0, end: 3, anchorFingerprint: 'paragraph-a1', anchorText: 'doi:10.1/test', before: 'doi', replacementText: 'https://doi.org/10.1/test', targetUrl: 'https://doi.org/10.1/test', action: 'make-hyperlink', confirmed: true }],
  }],
...
describe('Repair Contract request policy', () => {
  ...
  it('prihvaca samo ograniceni lineSpacing i boolean deep za aktualni footnote typography recept', () => {
    expect(parseContractRequests([request('footnote-typography-fixer', {
      fontName: 'Arial',
      lineSpacing: 1,
      deep: true,
    ...
    expect(parseContractRequests([request('footnote-typography-fixer', {
      fontName: 'Arial',
      deep: 'true',
    })])).toMatchObject({ ok: false, issues: [{ code: 'invalid-param' }] });
    for (const lineSpacing of [0, -1, 5.01, '1']) {
      expect(parseContractRequests([request('footnote-typography-fixer', {
        lineSpacing,
      })]), String(lineSpacing)).toMatchObject({
        ok: false,
        issues: [{ code: 'invalid-param' }],
      });
    }
  });
  ...
  it('odbija dodatni kljuc na svakoj dubini jednostavnih ugnijezdenih parametara', () => {
    const cases: Array<{
      fixerId: Exclude<FixerId, 'footer-page-fixer'>;
      params: Record<string, unknown>;
      mutate: (params: any) => void;
    }> = [
      {
        fixerId: 'paragraph-spacing-fixer',
        params: { styleRules: [{ styleId: 'Normal', beforeTwentieths: 0, keepLines: true }] },
        mutate: (params) => { params.styleRules[0].command = 'Start-Process'; },
      },
      {
        fixerId: 'paragraph-spacing-fixer',
        params: { targets: [{ paragraphIndex: 1, firstLineTwips: 720, clearDirectIndent: true }] },
        mutate: (params) => { params.targets[0].command = 'Start-Process'; },
      },
      {
        fixerId: 'page-numbering-fixer',
        params: { targets: [{ sectionIndex: 0, fmt: 'lowerRoman', start: 1 }] },
        mutate: (params) => { params.targets[0].command = 'Start-Process'; },
      },
      {
        fixerId: 'section-insert-fixer',
        params: { target: { introParagraphIndex: 2, align: 'center', frontFmt: 'lowerRoman' } },
        mutate: (params) => { params.target.command = 'Start-Process'; },
      },
      {
        fixerId: 'toc-field-fixer',
        params: { target: { sadrzajParagraphIndex: 2 } },
        mutate: (params) => { params.target.command = 'Start-Process'; },
      },
      {
        fixerId: 'heading-format-fixer',
        params: { targets: [{ level: 1, sizeHalfPoints: 32, bold: true, italic: false, alignLeft: true }] },
        mutate: (params) => { params.targets[0].command = 'Start-Process'; },
      },
      {
        fixerId: 'heading-style-fixer',
        params: { targets: [{ paragraphIndex: 1, anchorText: 'Uvod', level: 1, numbered: true }], options: { pageBreakLevels: [1], numbering: { maxLevel: 3, format: 'decimal', trailingDot: true } } },
        mutate: (params) => { params.targets[0].command = 'Start-Process'; },
      },
      {
        fixerId: 'heading-style-fixer',
        params: { targets: [], options: { pageBreakLevels: [1], numbering: { maxLevel: 3, format: 'decimal', trailingDot: true } } },
        mutate: (params) => { params.options.command = 'Start-Process'; },
      },
      {
        fixerId: 'heading-style-fixer',
        params: { targets: [], options: { pageBreakLevels: [1], numbering: { maxLevel: 3, format: 'decimal', trailingDot: true } } },
        mutate: (params) => { params.options.numbering.command = 'Start-Process'; },
      },
      {
        fixerId: 'title-page-fixer',
        params: { paragraphCount: 1, lines: [{ role: 'title', text: 'Naslov', group: 1, style: { font: 'Arial', sizePt: 14, bold: true, italic: false, uppercase: false, align: 'center' } }], marginsCm: { top: 2, right: 2.5, bottom: 2, left: 2.5 } },
        mutate: (params) => { params.lines[0].command = 'Start-Process'; },
      },
      {
        fixerId: 'title-page-fixer',
        params: { paragraphCount: 1, lines: [{ text: 'Naslov', style: { bold: true } }] },
        mutate: (params) => { params.lines[0].style.command = 'Start-Process'; },
      },
      {
        fixerId: 'title-page-fixer',
        params: { paragraphCount: 1, lines: [{ text: 'Naslov' }], marginsCm: { top: 2 } },
        mutate: (params) => { params.marginsCm.command = 'Start-Process'; },
      },
    ];

    for (const testCase of cases) {
      expect(parseContractRequests([request(testCase.fixerId, testCase.params)]), testCase.fixerId).toMatchObject({ ok: true });
      const injected = structuredClone(testCase.params);
      testCase.mutate(injected);
      expect(parseContractRequests([request(testCase.fixerId, injected)]), testCase.fixerId).toMatchObject({ ok: false });
    }
  });
