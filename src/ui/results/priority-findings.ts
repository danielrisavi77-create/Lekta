...
export function priorityFindingHtml(finding: VisualFindingModel, repairAvailable: boolean): string {
...
export function priorityFindingsHtml(findings: readonly VisualFindingModel[], repairAvailable: boolean): string {
  ...
  return findings.slice(0, 3).map((finding) => priorityFindingHtml(finding, repairAvailable)).join('');
}
