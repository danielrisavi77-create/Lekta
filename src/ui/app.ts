...
import { beginResultRender, settleResultRender } from './result-ready-signal';
...
// Zeton zadnjeg crtanja rezultata (vidi src/ui/result-ready-signal.ts). Zivi ovdje jer se izdaje u
// renderResult, a trosi ga asinkroni repair lanac koji krece iz renderSubmissionChecklist.
let _resultReadyToken=0;
...
function renderSubmissionChecklist(r: any){
 ...
 // SIGNAL SPREMNOSTI (`#resultView[data-result-ready]`) objavljuje se na OBA izlaza, jer je i
 // preskok gradnje zavrsetak crtanja; objava samo na jednoj grani ostavila bi ekran trajno na "0".
 const _tok=_resultReadyToken;
 if(r&&repairPanelForResult===r){settleResultRender(runtimeDocument(),_tok);return}
 // `.finally` NA OBECANJU, ne unutar renderRepairSection: tijelo te funkcije u svom `finally`
 // ponovno crta #repairEntry i cijeli #resultCockpit, pa bi signal postavljen iznutra jos uvijek
 // pao PRIJE zadnje izmjene DOM-a. Ovdje se ceka da cijeli lanac zavrsi.
 void renderRepairSection(r)
  .catch((e: any)=>console.error('Repair panel:',e))
  .finally(()=>{settleResultRender(runtimeDocument(),_tok)});
}
