...
function animateBars(){const A=_animate();runtimeDocument().querySelectorAll('#categoryGrid .bar > i').forEach((el: any)=>{const p=Number(el.dataset.fill)||0;if(motionReduced()||typeof A!=='function'){el.style.width=p+'%';return}resultAnimations.track(A(el,{width:['0%',p+'%']},{duration:.8,ease:[.22,1,.36,1],delay:.12}))})}
