...
async function proceedReportCheckout(productId: any,token: any,consent: any){const ub=$('#unlockReport');if(ub)ub.disabled=true;toast('Otvaram sigurno plaćanje…');try{const out=await createCheckout({endpoint:String(productionConfig.checkoutEndpoint).trim()},token,productId,null,fetch,consent);if(out.kind==='ok'){trackEvent('checkout_started',{product:productId});await (await loadStripeModal()).openStripePaymentModal({out,toast,trapModal,releaseModal,config:productionConfig,token,onPaid:handleUnlockReport});return}if(out.kind==='unauthorized')toast('Za kupnju paketa potrebna je prijava.');else if(out.kind==='forbidden')toast('Ovaj paket trenutačno nije dostupan.');else if(out.kind==='not_found')toast('Paket nije pronađen u katalogu.');else toast('Plaćanje trenutačno nije dostupno.')}catch(e: any){toast('Greška pri otvaranju plaćanja.')}finally{if(ub)ub.disabled=false}}
