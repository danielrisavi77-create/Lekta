...
import { setSiteChromePlate } from '../shared/site-chrome';
// BL-P0-05-4: DOCX parser se uvozi LIJENO (dinamicki import) tek nakon odabira datoteke,
// pa njegov kod ispada iz landing chunka.
...
let _corpusContribClientPromise: Promise<typeof import('../report/corpus-contribution-client')>|null=null;
function loadCorpusContribClient(){return _corpusContribClientPromise??=import('../report/corpus-contribution-client')}
...
let _stripePaymentPromise: any=null;
function loadStripePayment(){return _stripePaymentPromise??=import('../report/stripe-payment')}
...
/**
 * NARUDZBA RUCNE OBRADE (obrazac `#orderModal`) je opceniti kontakt/upit, BEZ vlastitog cjenika.
 *
 * Do Z11 je ovdje zivio cetvrti popis cijena (`PACKAGES`, 39/69/99 EUR), paralelan sa STVARNOM
 * naplatom (tablica `products` u bazi, `src/catalog/products-catalog.ts`, `slot_*`/`pass_*`/`bundle_*`).
 * Nijedan payment provider ni checkout te tri stavke nije poznavao, pa je narudzba obecavala paket
 * koji naplata ne bi znala isporuciti. Popis je uklonjen; obrazac sad samo prima ime, e-mail, rok,
 * napomenu i dokument, bez odabira paketa ni cijene. `data/packages.json` je iz istog razloga
 * ranije obrisan (nitko ga nije citao kao podatak).
 */
...
// Naslovnica obrasca je uklonjena 2026-09-07 (rez `/rad/`): carobnjak je vidljiv odmah, jer
// korisnik ondje dolazi s dokumentom. Time su otisla i dva pozivatelja s `pick:true`
// (`#paperCoverBtn`, `#uploadCtaBtn`) i klasa `.lek-engaged`, koja je gejtirala vidljivost.
// Ostaje SAMO dovodjenje fokusa na zonu, za sidro `#analyzer` i za otvaranje iz povijesti.
...
function saveOrderReceipt(order: any){const arr=safeStorageGet(STORAGE_KEYS.orders,[])||[];arr.unshift({orderId:order.orderId,createdAt:order.createdAt,status:order.status,profile:order.analysis?.profile||order.requestedProfile?.unit||null});safeStorageSet(STORAGE_KEYS.orders,arr.slice(0,10))}
...
async function renderRepairHistoryList(){const el=$('#repairHistoryList');if(!el)return;el.innerHTML='<div class="empty">Učitavam…</div>';const token: any=await ensureAccessToken();if(authConfigured()&&!token){el.innerHTML='<div class="empty">Za pregled popravaka potrebna je prijava.</div>';openAuth(()=>renderRepairHistoryList());return}let jobs: any[]=[];try{jobs=await fetchRepairJobs(repairHistoryConfig(),token||'')}catch(e: any){el.innerHTML='<div class="empty">Popravke trenutačno nije moguće učitati.</div>';return}
 // Kanal A: prilozi korpusu po poslu, samo kad je znacajka ukljucena; pad dohvata ne rusi popis (nema gumba, nista vise).
 const corpusByJob=new Map<string,string>();if(productionConfig?.corpusContribution===true){try{const cc=await loadCorpusContribClient();for(const c of await cc.fetchActiveCorpusContributions(cc.corpusContributionConfigFrom(productionConfig),token||''))if(c.repairJobId)corpusByJob.set(c.repairJobId,c.id)}catch{}}
if(!jobs.length){el.innerHTML='<div class="empty">Još nemaš spremljenih popravaka. Pojave se ovdje nakon "Popravi sve".</div>';return}el.innerHTML=jobs.map((j: any)=>{const tier=tierFor(j.workType),date=j.createdAt?new Date(j.createdAt).toLocaleString('hr-HR'):'',mb=j.resultBytes?`${(j.resultBytes/1024/1024).toFixed(2)} MB`:'';return`<article class="history-card"><div class="history-head"><div><h4>${escapeHtml(j.label||'Popravljeni rad')}</h4><p>${escapeHtml(tier?tier.label:j.workType)}</p></div></div><div class="history-meta"><span>${escapeHtml(date)}</span>${j.changesCount!=null?`<span>${escapeHtml(_plIzmjena(j.changesCount))}</span>`:''}${mb?`<span>${escapeHtml(mb)}</span>`:''}</div><div class="history-actions">${j.deleting?'<span class="muted">Brisanje u tijeku…</span>':`<button class="btn btn-secondary btn-sm" data-repair-download="${escapeHtml(j.id)}" data-path="${escapeHtml(j.resultPath)}" data-name="${escapeHtml(repairDownloadName(j))}">Preuzmi</button><button class="btn btn-ghost btn-sm" data-repair-del="${escapeHtml(j.id)}">Obriši</button>${corpusByJob.has(j.id)?`<button class="btn btn-ghost btn-sm" data-corpus-withdraw="${escapeHtml(corpusByJob.get(j.id)||'')}">Povuci prilog korpusu</button>`:''}`}</div></article>`}).join('');window.__lektaIcons?.()}
async function handleRepairHistoryAction(e: any){const dl=e.target.closest('[data-repair-download]'),del=e.target.closest('[data-repair-del]'),cw=e.target.closest('[data-corpus-withdraw]');if(!dl&&!del&&!cw)return;
 if(cw){const cid=cw.dataset.corpusWithdraw;if(!confirm('Povući privolu za prilog korpusu? Pseudonimizirana kopija bit će obrisana sa servera.'))return;const t: any=await ensureAccessToken();if(authConfigured()&&!t){openAuth(()=>renderRepairHistoryList());return}cw.disabled=true;const cc=await loadCorpusContribClient();const out=await cc.withdrawCorpusContribution(cc.corpusContributionConfigFrom(productionConfig),t||'',cid);if(out.ok){toast('Privola je povučena, kopija je obrisana.');void renderRepairHistoryList()}else{cw.disabled=false;toast('Povlačenje trenutačno nije moguće.')}return}const token: any=await ensureAccessToken();if(authConfigured()&&!token){openAuth(()=>renderRepairHistoryList());return}if(dl){const path=dl.dataset.path,orig=dl.textContent;dl.disabled=true;dl.textContent='Pripremam…';
...
//
// Do sada se kartica prikazivala SAMO kad profil nije verificiran (`needsProfileConfirmation`),
// pa je glavni sadrzaj koraka bio formular s devet selectova, a potvrda iznimka. Vlastiti UX
// principi traze obrnuto: "jedan redak s popunjenim defaultom, primarna akcija Potvrdi, promjena
// sekundarna i skrivena iza promijeni, cilj nula do jedan tap".
//
// TON OVISI O POUZDANOSTI DETEKCIJE, i to nije ukras. `_profileConfirmed` se postavlja iz
// `isConfidentDetection(ctx)`, koji je istinit SAMO kad je iz teksta prepoznat studij. Kad nije,
// izbornik studija stoji na alfabetski prvom fallbacku iz `populatePrograms`. Kartica koja bi to
// prikazala jednako kao prepoznat profil tvrdila bi korisniku cija se pravila primjenjuju, a ne
// zna. Zato tada vodi "Promijeni", a potvrda je sekundarna.
function renderAnalyzeSummary(p: any){
 const el=$('#analyzeProfile');if(!el)return;
 const sel=p.selection,sm=(PROFILE_STATUS as any)[p.statusKey]||PROFILE_STATUS.generic;
 el.innerHTML=renderProfileCard({
  naslov:sel.unit||sel.institution||'',studij:sel.program,vrsta:workTypeLabel(sel.workType),
  citiranje:sel.citationStyle,akademskaGodina:p.verifiedAt?academicYearFromDate(p.verifiedAt):null,
  statusKey:p.statusKey,statusLabel:sm.label,nesiguran:!!selectedDocx&&!_profileConfirmed,
 },escapeHtml);
}
...
// LIST PROFILA. Isti put kao ostali modali (`trapModal`/`releaseModal`): zamka fokusa, inertna
// pozadina, povratak fokusa na okidac. Do 2026-09-07 su se kontrole samo OTKRIVALE na mjestu
// klasom `lek-izmjena`, pa nije bilo ni zamke fokusa ni izlaza tipkovnicom, a Tab je nastavljao
// u sadrzaj iza njih.
function openProfileSheet(){$('#profileSheet')?.classList.remove('hidden');trapModal($('#profileSheet'))}
function closeProfileSheet(){$('#profileSheet')?.classList.add('hidden');releaseModal($('#profileSheet'))}
...
/* PRIVOLA OTKAZUJE ZATVORENO. Do 2026-09-05 je bez modala odmah zvala onConfirm s immediateDelivery:true i
   withdrawalWaived:true, dakle biljezila odricanje od prava na odustajanje BEZ IJEDNOG KLIKA, na svakoj
   stranici koja modal nema (na /rad/ je taj put bio dostizan). Bez modala nema privole, nema ni narudzbe. */
function openCheckoutConsent(summary: any,onConfirm: any){const m=$('#checkoutConsentModal');if(!m){toast('Potvrda narudžbe nije dostupna na ovoj stranici, pa narudžba nije poslana.');return}const chk=$('#checkoutConsentCheck'),btn=$('#confirmCheckoutConsent');$('#checkoutConsentSummary').textContent=summary;$('#checkoutConsentText').textContent=CHECKOUT_CONSENT_TEXT;chk.checked=false;btn.disabled=true;chk.onchange=()=>{btn.disabled=!chk.checked};btn.onclick=()=>{if(!chk.checked)return;const consent={immediateDelivery:true,withdrawalWaived:true,text:CHECKOUT_CONSENT_TEXT,timestamp:new Date().toISOString(),termsVersion:TERMS_VERSION};closeCheckoutConsent();onConfirm(consent)};m.classList.remove('hidden');trapModal(m);window.__lektaIcons?.()}
...
// PLACANJE U STRANICI (F18, 2026-09-23). Prije je ovdje bio `location.href = out.checkoutUrl`
// prema hosted checkoutu Merchant of Record providera. Stripe Payment Element se montira u
// modal, pa korisnik ne napusta rezultat analize; nakon uspjeha nastavlja se ISTIM putem kao
// prije kad se vracao s placanja, dakle `handleUnlockReport()` koji provjeri pravo i otkljuca
// izvjestaj. Nacini placanja koji obavezno traze bankovnu stranicu i dalje odu na `returnUrl`.
async function proceedReportCheckout(productId: any,token: any,consent: any){const ub=$('#unlockReport');if(ub)ub.disabled=true;toast('Otvaram sigurno plaćanje…');try{const out=await createCheckout({endpoint:String(productionConfig.checkoutEndpoint).trim()},token,productId,null,fetch,consent);if(out.kind==='ok'){trackEvent('checkout_started',{product:productId});await openStripePayment(out,productId);return}if(out.kind==='unauthorized')toast('Za kupnju paketa potrebna je prijava.');else if(out.kind==='forbidden')toast('Ovaj paket trenutačno nije dostupan.');else if(out.kind==='not_found')toast('Paket nije pronađen u katalogu.');else toast('Plaćanje trenutačno nije dostupno.')}catch(e: any){toast('Greška pri otvaranju plaćanja.')}finally{if(ub)ub.disabled=false}}
function closeStripePayment(){const m=$('#stripePaymentModal');if(!m)return;m.classList.add('hidden');const host=$('#stripePaymentElement');if(host)host.innerHTML='';releaseModal(m)}
async function openStripePayment(out: any,productId: any){const m=$('#stripePaymentModal'),host=$('#stripePaymentElement'),btn=$('#confirmStripePayment'),status=$('#stripePaymentStatus');if(!m||!host||!btn){toast('Plaćanje nije dostupno na ovoj stranici.');return}
 const mod: any=await loadStripePayment();let stripe: any;
 try{const factory=await mod.loadStripeJs();stripe=factory(out.publishableKey)}catch(e: any){toast('Plaćanje se nije učitalo. Pokušaj ponovno.');return}
 host.innerHTML='';if(status)status.textContent='';
 const {elements}=mod.mountPaymentElement({stripe,clientSecret:out.clientSecret,container:host});
 m.classList.remove('hidden');trapModal(m);window.__lektaIcons?.();
 btn.disabled=false;btn.onclick=async()=>{btn.disabled=true;if(status)status.textContent='Potvrđujem plaćanje…';
  try{const res=await mod.confirmPayment({stripe,elements,returnUrl:location.href});
   if(res.kind==='ok'){closeStripePayment();toast('Plaćanje je uspjelo. Otključavam izvještaj…');trackEvent('purchase_completed',{product:productId});await handleUnlockReport();return}
   if(res.kind==='redirected'){if(status)status.textContent='Plaćanje se dovršava kod banke.';return}
   if(status)status.textContent=res.message}catch(e: any){if(status)status.textContent='Greška pri potvrdi plaćanja.'}finally{btn.disabled=false}}}
...
function openOrder(brief?: any){if(!paidOffersLive()){toast('Ručno uređivanje bit će uskoro dostupno. Za rani upit: '+(productionConfig.contactEmail||'kontakt uskoro'));return}clearOrderStatus();$('#orderConsent').checked=false;renderProductionState();updateOrderFileMeta();const _notes=$('#orderNotes');if(brief&&_notes&&!_notes.value.trim())_notes.value=brief;$('#orderModal').classList.remove('hidden');trapModal($('#orderModal'));trackEvent('order_opened',{provider:productionConfig.paymentProvider});setTimeout(()=>$('#orderName').focus(),50)}function closeOrder(){$('#orderModal')?.classList.add('hidden');releaseModal($('#orderModal'))}
async function submitOrder(){const name=$('#orderName').value.trim(),email=$('#orderEmail').value.trim(),file=selectedOrderFile(),consent=$('#orderConsent').checked;if(!name||!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)){toast('Upiši ime i valjanu e-mail adresu.');return}if(!consent){toast('Za slanje narudžbe potrebno je prihvatiti uvjete i obavijest o privatnosti.');return}if(file&&(!file.name.toLowerCase().endsWith('.docx')||file.size>productionConfig.uploadMaxBytes)){setOrderStatus('error',`Dokument mora biti .docx i ne smije biti veći od ${Math.round(productionConfig.uploadMaxBytes/1024/1024)} MB. Narudžbu možeš poslati bez datoteke i naknadno dogovoriti drugi siguran kanal.`);return}await ensureRulesForCurrentSelection(currentDefinitionId,ensureProfileRules);const orderId=makeOrderId(),profile=currentProfile().p,order={orderId,createdAt:new Date().toISOString(),customer:{name,email},deadline:$('#orderDeadline').value||null,notes:$('#orderNotes').value.trim(),marketingConsent:$('#orderMarketing').checked,termsVersion:TERMS_VERSION,document:file?{name:file.name,size:file.size,type:file.type}:null,analysis:currentResult?{file:currentResult.file.name,score:currentResult.score,profile:currentResult.profile,profileStatus:currentResult.profileStatus,fingerprint:currentResult.profileFingerprint||null,selection:currentResult.selection}:null,requestedProfile:profile.selection,status:'PENDING'};trackEvent('order_submit_attempt',{profileId:profile.definitionId||profile.id,workType:profile.selection.workType,provider:productionConfig.paymentProvider});const button=$('#submitOrder');button.disabled=true;button.textContent='Šaljem…';setOrderStatus('pending','Pripremam sigurnu predaju narudžbe…');try{if(!productionStatus().active){order.status='LOCAL_DRAFT';downloadBlob(JSON.stringify(order,null,2),'application/json',`Lekta-narudzba-${orderId}.json`);saveOrderReceipt(order);setOrderStatus('success',`<strong>Testni zapis je izrađen.</strong> Narudžba ${escapeHtml(orderId)} nije poslana niti je dokument prenesen. Preuzeta je lokalna JSON datoteka za provjeru integracije.`);toast('Testna narudžba spremljena je lokalno.');return}const fd=new FormData();fd.append('form-name','lekta-orders');fd.append('order_id',orderId);fd.append('name',name);fd.append('email',email);fd.append('deadline',order.deadline||'');fd.append('notes',order.notes);fd.append('profile',JSON.stringify(order.requestedProfile));fd.append('fingerprint',order.analysis?.fingerprint||'');fd.append('marketing_consent',String(order.marketingConsent));fd.append('terms_version',TERMS_VERSION);if(file)fd.append('document',file,file.name);const response=await fetch(productionConfig.orderEndpoint,{method:'POST',body:fd,headers:{Accept:'application/json, text/plain, */*'}});if(!response.ok)throw new Error(`HTTP ${response.status}`);order.status='RECEIVED';saveOrderReceipt(order);const payBase=productionConfig.paymentLinks?.default||'',payUrl=buildPaymentUrl(payBase,email,orderId,'manual'),payment=payUrl?`<div class="payment-action"><a class="btn btn-primary btn-sm" id="paymentContinue" href="${escapeHtml(safeHref(payUrl))}" target="_blank" rel="noopener">Nastavi na sigurno plaćanje →</a></div>`:`<div class="payment-action"><strong>Plaćanje još nije povezano.</strong> Potvrda i upute bit će poslane na navedeni e-mail.</div>`;setOrderStatus('success',`<strong>Narudžba ${escapeHtml(orderId)} je zaprimljena.</strong> ${file?'Dokument je uključen u sigurnu predaju.':'Narudžba je zaprimljena bez dokumenta.'}${payment}`);trackEvent('order_submitted',{profileId:profile.definitionId||profile.id,provider:productionConfig.paymentProvider});setTimeout(()=>{const a=$('#paymentContinue');if(a)a.onclick=()=>trackEvent('payment_redirect',{provider:productionConfig.paymentProvider})},0)}catch(err: any){order.status='FAILED';setOrderStatus('error',`Narudžbu nije bilo moguće poslati (${escapeHtml(err.message||'mrežna pogreška')}). Dokument nije označen kao zaprimljen. Pokušaj ponovno ili kontaktiraj ${escapeHtml(productionConfig.contactEmail||'podršku nakon konfiguracije kontakta')}.`)}finally{button.disabled=false;button.textContent='Pošalji narudžbu'}}
/**
