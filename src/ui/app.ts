...
let _stripeModalPromise: any=null;
function loadStripeModal(){return _stripeModalPromise??=import('./stripe-payment-modal')}
...
// PLACANJE U STRANICI (F18): modal Payment Elementa zivi u src/ui/stripe-payment-modal.ts (i sam
// se zatvara), pa je ovdje samo lijeni poziv. Na uspjeh ide handleUnlockReport, kao i dosad.
async function proceedReportCheckout(productId: any,token: any,consent: any){const ub=$('#unlockReport');if(ub)ub.disabled=true;toast('Otvaram sigurno plaćanje…');try{const out=await createCheckout({endpoint:String(productionConfig.checkoutEndpoint).trim()},token,productId,null,fetch,consent);if(out.kind==='ok'){trackEvent('checkout_started',{product:productId});await (await loadStripeModal()).openStripePaymentModal({out,productId,toast,trapModal,releaseModal,trackEvent,onPaid:handleUnlockReport});return}if(out.kind==='unauthorized')toast('Za kupnju paketa potrebna je prijava.');else if(out.kind==='forbidden')toast('Ovaj paket trenutačno nije dostupan.');else if(out.kind==='not_found')toast('Paket nije pronađen u katalogu.');else toast('Plaćanje trenutačno nije dostupno.')}catch(e: any){toast('Greška pri otvaranju plaćanja.')}finally{if(ub)ub.disabled=false}}
