...
import { typoLint, typoLintSummary, KIND_LABELS_HR, KIND_DOCUMENT_WIDE } from '../tools/typo-lint';
...
import { VERIFIED_PROFILE_REGISTRY, LEGAL_DEPARTMENT_REGISTRY, BASE_PROFILES, FPZG_PARTIAL, PROFILE_STATUS, PROFILE_AUTHORITY, ensureProfileRules } from '../profiles/profile-registry';
...
import { detectContextFromText, needsProfileConfirmation } from './profile-detect';
...
const PRICING_TIERS=[
 ...
 {id:'perwork',name:'Provjera po radu',price:'od 3,99 €',featured:true,desc:'Puni izvještaj i plan ispravaka, cijena prema vrsti rada.',features:['Sve iz besplatne provjere','Puni izvještaj i plan ispravaka','Tehnička provjera spremnosti (oblikovanje, struktura, citiranje) i preflight PDF-a','Seminarski 3,99 · Završni 5,99 · Diplomski 9,99 · Doktorski 24,99 €'],cta:{href:'#analyzer',label:'Provjeri rad'}},
...
function bind(){
 ...
 $('#dropzone').addEventListener('drop',(e: any)=>setFile(e.dataTransfer.files[0]));$('#removeFile').onclick=(e: any)=>{e.stopPropagation();setFile(null)};
 $('#institutionSelect').onchange=()=>{populateUnits();syncProfileContext();updatePackageUi()};$('#unitSelect').onchange=()=>{populatePrograms();syncProfileContext();updatePackageUi()};$('#programSelect').onchange=()=>{autoSelectWorkType();syncProfileContext();updatePackageUi()};$('#workType').onchange=()=>{syncProfileContext();updatePackageUi()};$('#workVariant').onchange=()=>{syncProfileContext();updatePackageUi()};$('#departmentSelect').onchange=syncProfileContext;$('#methodologySelect').onchange=syncProfileContext;$('#citationStyle').onchange=()=>{updateProfile();savePreferences()};$('#docLanguage').onchange=savePreferences;$('#strictness').onchange=savePreferences;$('#submissionPhase').onchange=()=>{savePreferences();updateSubmissionContextFields();updatePackageUi();if(currentResult)renderSubmissionChecklist(currentResult)};$('#fpzgCohort').onchange=()=>{populateFpzgDeadlineOptions();savePreferences();if(currentResult)renderSubmissionChecklist(currentResult)};$('#fpzgDeadline').onchange=()=>{savePreferences();if(currentResult)renderSubmissionChecklist(currentResult)};$('#mentorOverride').onchange=()=>{$('#customSettings').classList.toggle('hidden',!$('#mentorOverride').checked);updateProfile();savePreferences()};$('#analyzeBtn').onclick=runAnalysis;$('#cancelAnalysisBtn')&&($('#cancelAnalysisBtn').onclick=cancelAnalysis);$('#demoBtn').onclick=()=>runDemo('fpzg');$('#heroDemoBtn').onclick=()=>{document.querySelector('#analyzer')?.scrollIntoView();setTimeout(()=>runDemo('fpzg'),300)};
 ...
 $('#issueFilters').onclick=(e: any)=>{const b=e.target.closest('[data-filter]');if(!b)return;$$('.filter-btn').forEach(x=>x.classList.toggle('active',x===b));renderUnifiedIssues(b.dataset.filter)};
...
async function applyDetectedContext(file: any){
 ...
 syncProfileContext();savePreferences();_profileConfirmed=true;
...
function sanitizeEventData(data: any){const allowed: any={};for(const[k,v]of Object.entries(data||{})){if(['event','package','profileId','workType','scoreBand','provider','source','total','found','missing','flagged','checked'].includes(k)&&['string','number','boolean'].includes(typeof v))allowed[k]=v}return allowed}
...
function currentProfile(){
 ...
 // Lagani rad (seminar/projekt/clanak) BEZ posebnog profila: opci baseline ne smije traziti Sadrzaj
 // ni teza-strukturu (kratki seminar ih cesto nema); oblikovanje/citiranje ostaju kao opca preporuka.
 const lightBaseline=!definition&&['seminar','project','article'].includes(workType);if(lightBaseline){base.requireToc=false}
 ...
 base.statusKey=definition?.status||(u.status==='verified'?'generic':u.status);base.status=((PROFILE_STATUS as any)[base.statusKey]||fallbackStatus).label;base.verified=base.statusKey==='verified';base.citation=$('#citationStyle').value;base.accessDate=definition?.rules?.accessDate??cit.accessDate;base.citationMode=cit.mode;
...
// BL-P0-05-1: teska profilna pravila su lazy (ensureProfileRules ucita heavy chunk i spoji u registry).
// updateProfile cita currentProfile()->definition.rules pa mora pricekati merge; picker/hero rade odmah.
...
function renderNextSteps(r: any){const el=$('#nextSteps');if(!el)return;if(!r){el.classList.add('hidden');el.innerHTML='';return}
 // Na DEMU se ne nudi ni otkljucavanje ni narudzba: kupnja se vodi na STVARNI rad, ne na
 // fabricirani uzorak (lektorBrief bi inace pretpopunio narudzbu izmisljenim nalazima).
 const isDemo=!!r.demo,unlockable=!isDemo&&reportEndpointConfigured()&&!r.fullReport,manual=!isDemo&&paidOffersLive(),offeringsExist=reportEndpointConfigured()||paidOffersLive();
 const cards=[
  `<button class="next-step" type="button" data-next="plan"><b>Popravi sam prema planu</b><span>Otvara Plan ispravaka: problemi poredani po prioritetu, s konkretnim uputama.</span></button>`,
  `<button class="next-step" type="button" data-next="download"><b>Preuzmi izvještaj</b><span>HTML izvještaj za sebe ili mentora${paywallGateActive()?' (besplatni sažetak s vodenim žigom)':''}.</span></button>`,
  unlockable?`<button class="next-step" type="button" data-next="unlock"><b>Otključaj puni izvještaj</b><span>Automatska provjera bez ograničenja: svi problemi, detaljne tablice, bez žiga.</span></button>`:'',
  manual?`<button class="next-step" type="button" data-next="order"><b>Naruči ručno sređivanje</b><span>Čovjek uredi dokument prema nalazima; dokument šalješ tek u narudžbi, uz privolu.</span></button>`:''
 ].filter(Boolean).join('');
 const diff=isDemo?(offeringsExist?`<p class="ns-note"><strong>Ovo je primjer.</strong> Puni izvještaj i ručno sređivanje vežu se uz tvoj stvarni rad: učitaj dokument i pokreni analizu.</p>`:`<p class="ns-note">Ovo je primjer rezultata; učitaj svoj rad za stvarnu provjeru. Plaćene opcije bit će uskoro dostupne.</p>`):(unlockable||manual)?`<p class="ns-note"><strong>Razlika:</strong> puni izvještaj je i dalje automatska provjera (potpun popis nalaza, serverski potvrđen, bez vodenog žiga); ručno sređivanje znači da čovjek uređuje tvoj dokument prema tim nalazima.</p>`:`<p class="ns-note">Plaćene opcije (puni izvještaj, ručno sređivanje) bit će uskoro dostupne; do tada je cijela automatska provjera besplatna.</p>`;
 el.innerHTML=`<h3>Što dalje?</h3><div class="next-steps-grid">${cards}</div>${diff}<p class="ns-note">Želiš precizniju provjeru? <button type="button" class="linklike" id="nsPrecise">Otvori dodatne opcije</button> (stil citiranja, jezik, strogost, faza provjere) i ponovno pokreni analizu.</p>`;el.classList.remove('hidden');
 el.querySelectorAll('[data-next]').forEach((b: any)=>{b.onclick=()=>{const k=b.dataset.next;if(k==='plan'){openTab('action');$('#tabbtn-action')?.scrollIntoView({behavior:'smooth',block:'center'})}else if(k==='download')downloadHtmlReport();else if(k==='unlock')handleUnlockReport();else if(k==='order')openOrder('panic',lektorBrief())}});
 const p=$('#nsPrecise');if(p)p.onclick=()=>{$('#resultView').classList.add('hidden');$('#wizardView').classList.remove('hidden');const d=document.querySelector('.advanced-options');if(d)(d as any).open=true;document.querySelector('#analyzer')?.scrollIntoView({behavior:'smooth'});setTimeout(()=>$('#citationStyle')?.focus(),350)}}
...
// Vodjeni rezultat: "Najvaznije za popraviti" (top nalazi po ozbiljnosti) + primarni CTA (oznaceni
// pregled) + prekidac za sve detalje. Zamjenjuje redundantnu "Sto dalje?" kutiju i vodi prvog korisnika.
function renderResultGuide(r: any){
  const el=$('#resultGuide');if(!el)return;
  if(!r){el.classList.add('hidden');el.innerHTML='';return}
  const rank: any={error:3,warning:2,info:1},catNames: any={formatting:'Oblikovanje',structure:'Struktura',citations:'Citatnice',elements:'Elementi'};
  const sorted=(r.issues||[]).slice().sort((a: any,b: any)=>(rank[b.severity]||0)-(rank[a.severity]||0));
  const top=sorted.filter((x: any)=>x.severity==='error'||x.severity==='warning').slice(0,4);
  const list=top.length?top:sorted.slice(0,3);
  const items=list.map((x: any,i: number)=>`<li class="priority-item priority-item--${escapeHtml(x.severity)}"><button type="button" data-goto-issues><span class="pri-rank" aria-hidden="true">${i+1}</span><span class="pri-dot" aria-hidden="true"></span><span class="pri-title">${escapeHtml(x.title)}</span><span class="pri-where">${escapeHtml(catNames[x.category]||x.category||'')}</span></button></li>`).join('');
  const hasPreview=!!r.preview?.paragraphs?.length;
  const body=list.length
    ?`<h3 class="guide-h">Najvažnije za popraviti</h3><ol class="priority-list">${items}</ol>`
    :`<div class="guide-clean">Nema kritičnih stavki za popraviti.${hasPreview?' Otvori pregled da provjeriš napomene u tekstu.':' Rad je tehnički uredan.'}</div>`;
  const previewCta=hasPreview?`<button class="btn btn-primary btn-lg" id="guideOpenPreview" type="button">Otvori označeni pregled →</button>`:'';
  ...
  el.innerHTML=`${body}${scoreBreakdownHtml(r)}<div class="guide-cta">${previewCta}${shareCta}<button class="tab-toggle guide-more" id="resultDetailsToggle" type="button" aria-expanded="false" aria-controls="resultDetails">${list.length?'Prikaži sve detalje i provjere':'Prikaži detalje'} <i data-lucide="chevron-down"></i></button></div>`;
  el.classList.remove('hidden');
  $('#guideOpenPreview')?.addEventListener('click',()=>{void openPreview()});
  $('#guideShareScore')?.addEventListener('click',()=>{void shareScore(r)});
  el.querySelectorAll('[data-goto-issues]').forEach((b: any)=>{b.onclick=()=>{revealResultDetails();openTab('issues');$('#resultDetails')?.scrollIntoView({behavior:'smooth',block:'start'})}});
...
function wireFindingCards(root: any,r: any){
  root?.querySelectorAll('[data-finding-id]').forEach((card: any)=>{
    ...
    card.querySelector('[data-finding-repair]')?.addEventListener('click',()=>{void trackEvent('finding_opened',{category:finding.category});scrollToRepairPanel(r)});
...
function renderTriage(r: any){
  ...
  const el=$('#triagePanel');if(!el)return;const findings=topFindings(findingsFor(r).filter(f=>f.kind==='document'),3);
  if(!findings.length){el.classList.add('hidden');el.innerHTML='';return}
  el.classList.remove('hidden');el.innerHTML=`<h3 class="triage-head">Tri najvažnija koraka</h3><div class="finding-list finding-list--top">${findings.map(f=>findingCardHtml(f,repairAvailableFor(r))).join('')}</div>`;
  wireFindingCards(el,r);renderUnifiedActionPlan(r);renderUnifiedIssues('all');renderPhaseTwoResultViews(r);window.__lektaIcons?.();
}
function renderUnifiedActionPlan(r: any){
  const el=$('#actionPlan');if(!el)return;const findings=findingsFor(r).sort((a,b)=>a.priorityRank-b.priorityRank||a.originalIndex-b.originalIndex),gated=paywallGateActive(),visible=gated?findings.slice(0,TEASER_SAMPLE):findings,hidden=findings.length-visible.length;
  el.innerHTML=findings.length?`<div class="action-overview"><div class="action-stat"><span>OTVORENO</span><b>${findings.filter(f=>f.status==='open').length}</b></div><div class="action-stat"><span>RUČNO PROVJERENO</span><b>${findings.filter(f=>f.status==='confirmed').length}</b></div><div class="action-stat"><span>ZANEMARENO</span><b>${findings.filter(f=>f.status==='ignored').length}</b></div></div><div class="finding-list">${visible.map(f=>findingCardHtml(f,repairAvailableFor(r))).join('')}</div>${gated&&hidden>0?paywallLockHtml(`Cijeli plan ispravaka, još ${hidden} stavki`):''}`:'<div class="action-empty"><i data-lucide="check-circle"></i> Nema stavki za ispravak.</div>';
  wireFindingCards(el,r);wireLockCtas();
}
function renderUnifiedIssues(filter='all'){
  if(!currentResult)return;const el=$('#issuesList'),all=findingsFor(currentResult),gated=paywallGateActive(),visible=gated?all.slice(0,TEASER_SAMPLE):all,arr=visible.filter(f=>filter==='all'||f.severity===filter),hidden=all.length-visible.length;if(!el)return;
  $('#issueCountLabel').textContent=gated?`Prikazano ${arr.length} od ${all.length} (puni popis je u punom izvještaju)`:`Prikazano ${arr.length} od ${all.length}`;el.innerHTML=(arr.length?`<div class="finding-list">${arr.map(f=>findingCardHtml(f,repairAvailableFor(currentResult))).join('')}</div>`:'<div class="empty"><i data-lucide="check-circle"></i><p>Nema problema u ovoj kategoriji.</p></div>')+(gated&&hidden>0?paywallLockHtml(`Još ${hidden} ${hidden===1?'problem':'problema'} s uputama za ispravak`):'');wireFindingCards(el,currentResult);wireLockCtas();window.__lektaIcons?.();
}
function refreshFindingViews(r: any){renderTriage(r);renderUnifiedActionPlan(r);renderUnifiedIssues('all');renderPhaseTwoResultViews(r);$('#issueTabCount').textContent=`(${findingsFor(r).filter(f=>f.kind==='document'&&f.status!=='ignored').length})`}
...
function renderPhaseTwoResultViews(r: any){
  ...
  if(guide){
    ...
    guide.innerHTML=first
      ?`<h3 class="guide-h">Što prvo napraviti</h3><p class="guide-summary">${escapeHtml(summary)}. Počni s: <strong>${escapeHtml(first.title)}</strong>.</p><div class="guide-cta">${primary}<button class="tab-toggle guide-more" id="resultDetailsToggle" type="button" aria-expanded="false" aria-controls="resultDetails">Prikaži sve nalaze i provjere</button></div>`
      :'<div class="guide-clean">Nema otvorenih problema dokumenta. Prije predaje pregledaj ograničenja analize i posebne upute profila.</div>';
...
function scrollToRepairPanel(r: any){
  ...
  if(m){
    ...
    const act: any=m.querySelector('[data-repair-go]:not(:disabled),button:not(:disabled),a[href]');
...
function renderNetworkProof(){
  ...
  const msg=networkProofMessage(probe.stop());
...
function lektorBrief(){if(!currentResult)return'';const pick=[...currentResult.issues.filter((i: any)=>i.severity==='error'),...currentResult.issues.filter((i: any)=>i.severity==='warning')].slice(0,8),lines=pick.map(i=>`- [${i.severity==='error'?'VAŽNO':'UPOZORENJE'}] ${i.title}${i.where?` (${i.where})`:''}`);return`Brief iz Lekta analize: ${currentResult.profile}, ocjena ${currentResult.score??'-'}${currentResult.issues.length?`, ${currentResult.issues.length} nalaza`:''}.\n${lines.join('\n')}${currentResult.issues.length>pick.length?`\n(+ još ${currentResult.issues.length-pick.length} nalaza u izvještaju)`:''}`}
function renderActionPlan(r: any){const errors=(r.issues||[]).filter((x: any)=>x.severity==='error'),warnings=(r.issues||[]).filter((x: any)=>x.severity==='warning'),infos=(r.issues||[]).filter((x: any)=>x.severity==='info'),quick=warnings.filter((x: any)=>['formatting','elements'].includes(x.category)),structural=warnings.filter((x: any)=>!quick.includes(x)),manual=(r.details?.manualChecks||[]).map((x: any)=>({title:'Ručna provjera',detail:x,where:'Službene upute'}));const gated=paywallGateActive(),colLimit=gated?1:6;const _sctx={unitId:r.settings?.selectionIds?.unit||'',workType:r.settings?.workType||r.selection?.workType||'',program:r.selection?.program||''};const items=(arr: any,limit=colLimit)=>arr.slice(0,limit).map((x: any)=>{const _tool=suggestTool(x,_sctx);return `<div class="action-item"><strong>${escapeHtml(x.title)}</strong><p>${escapeHtml(diagText(x.detail))}</p><small>${escapeHtml(x.where||x.category||'')}</small>${_tool?`<a class="action-tool" href="${escapeHtml(safeHref(_tool.href))}" target="_blank" rel="noopener"><i data-lucide="wrench"></i> ${escapeHtml(_tool.label)} →</a>`:''}</div>`}).join('')||'<div class="action-empty"><i data-lucide="check-circle"></i> Nema stavki u ovoj skupini.</div>';$('#actionPlan').innerHTML=`<div class="action-overview"><div class="action-stat"><span>PRVO ISPRAVI</span><b>${errors.length}</b></div><div class="action-stat"><span>BRZE DORADE</span><b>${quick.length}</b></div><div class="action-stat"><span>RUČNO POTVRDI</span><b>${manual.length+infos.length}</b></div></div><div class="action-columns"><section class="action-column"><h3>1. Kritični problemi</h3><p>Stavke koje najviše utječu na tehničku usklađenost.</p>${items(errors)}</section><section class="action-column"><h3>2. Brze i strukturne dorade</h3><p>Oblikovanje, elementi i upozorenja koja se obično brzo popravljaju.</p>${items([...quick,...structural])}</section><section class="action-column"><h3>3. Ručna završna provjera</h3><p>Stavke koje parser ne može pouzdano odlučiti bez konteksta.</p>${items([...manual,...infos])}</section></div>${gated&&(errors.length+quick.length+structural.length+manual.length+infos.length)>3?paywallLockHtml('Cijeli plan ispravaka po prioritetima'):''}${(errors.length+warnings.length)>0&&paidOffersLive()?`<div class="handoff-strip"><div><strong>Ne stigneš sve sam?</strong><p>Predaj ljudskom uređivanju: sažetak nalaza (naslovi problema, ne tekst rada) automatski ide u narudžbu kao brief. Dokument prilažeš tek u narudžbi, uz izričitu privolu.</p></div><button class="btn btn-secondary btn-sm" type="button" id="handoffLektor">Predaj ljudskom uređivanju →</button></div>`:''}`;const _ho=$('#handoffLektor');if(_ho)_ho.onclick=()=>{openOrder('panic',lektorBrief());trackEvent('handoff_lektor_opened',{profileId:r.details?.profileDefinitionId||''})};if(gated)wireLockCtas()}
...
async function renderReferralShareBar(){const el=$('#referralShareBar');if(!el)return;el.innerHTML='';const c=authConfig();if(!c.supabaseUrl||!c.anonKey)return;const token: any=await resolveAccessToken();if(!token)return;try{await renderReferralShareSection({supabaseUrl:c.supabaseUrl,anonKey:c.anonKey,accessToken:token,appBaseUrl:location.origin||'',mountEl:el})}catch(e: any){}}
...
function renderRepairSection(r: any){
 ...
 const items=[...buildRepairableItems(r.checks||[],analyzedProfile,entries,{includeNonViolated:true}),...universalRepairableItems(r.issues||[]),...paragraphSpacingRepairableItem(r.checks||[],analyzedProfile),...pageNumberingRepairableItem(r,analyzedProfile),...footnoteSpacingRepairableItem(r.checks||[],analyzedProfile),...pageNumberAlignmentRepairableItem(r.checks||[],analyzedProfile),...introSectionRepairableItem(r,analyzedProfile),...tocFieldRepairableItem(r,analyzedProfile),...headingFormatRepairableItem(r.checks||[],analyzedProfile),...footnoteTypographyRepairableItem(r.checks||[],analyzedProfile)];
 if(!items.length) return;
...
function renderIssues(filter='all'){if(!currentResult)return;const gated=paywallGateActive(),all=currentResult.issues,visible=gated?all.slice(0,TEASER_SAMPLE):all,arr=visible.filter((i: any)=>filter==='all'||i.severity===filter),hidden=all.length-visible.length;$('#issueCountLabel').textContent=gated?`Prikazano ${arr.length} od ${all.length} (puni popis je u punom izvještaju)`:`Prikazano ${arr.length} od ${all.length}`;$('#issuesList').innerHTML=(arr.length?arr.map((i: any)=>`<article class="issue ${i.severity}"><div class="issue-icon">${i.severity==='error'?'<i data-lucide="alert-circle"></i>':i.severity==='warning'?'<i data-lucide="alert-triangle"></i>':'<i data-lucide="info"></i>'}</div><div><h4>${escapeHtml(i.title)}</h4><p>${escapeHtml(diagText(i.detail))}</p></div><span class="where">${escapeHtml(i.where||i.category)}</span></article>`).join(''):'<div class="empty"><i data-lucide="check-circle"></i><p>Nema problema u ovoj kategoriji. Sve stavke ovog filtra su uredne.</p></div>')+(gated&&hidden>0?paywallLockHtml(`Još ${hidden} ${hidden===1?'problem':hidden<5?'problema':'problema'} s uputama za ispravak`):'');wireLockCtas()}
...
function resetAnalyzer(){currentResult=null;analyzedProfile=null;if(preflightPanel){preflightPanel.dispose();preflightPanel=null;preflightPanelForResult=null}resetUnlockButton();withViewTransition(()=>{$('#resultView').classList.add('hidden');$('#progressView').classList.add('hidden');$('#wizardView').classList.remove('hidden')});setFile(null);setAuxFile('pdf',null);setAuxFile('metadata',null);setAuxFile('av',null);progress(0,'Pripremam analizu');document.querySelector('#analyzer')?.scrollIntoView({behavior:'smooth'})}
...
function openOrder(id='format',brief?: any){if(!paidOffersLive()){toast('Ručno uređivanje bit će uskoro dostupno. Za rani upit: '+(productionConfig.contactEmail||'kontakt uskoro'));return}currentPackage=id;$$('input[name="package"]').forEach(r=>r.checked=r.value===id);clearOrderStatus();$('#orderConsent').checked=false;renderProductionState();updateOrderFileMeta();const _notes=$('#orderNotes');if(brief&&_notes&&!_notes.value.trim())_notes.value=brief;$('#orderModal').classList.remove('hidden');trapModal($('#orderModal'));trackEvent('order_opened',{package:id,provider:productionConfig.paymentProvider});setTimeout(()=>$('#orderName').focus(),50)}function closeOrder(){$('#orderModal').classList.add('hidden');releaseModal($('#orderModal'))}
