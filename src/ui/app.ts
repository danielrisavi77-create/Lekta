...
import { focusResult as focusResultInDocument } from '../shared/result-a11y'; // BL-P1-02: fokus + SR-najava rezultata
...
function toast(msg: any){const wrap=$('#toastWrap');if(!wrap)return;const n=runtimeDocument().createElement('div');n.className='toast';n.textContent=msg;wrap.append(n);setTimeout(()=>n.remove(),3500)}
...
function bindAnalyzerRoute(doc: Document){
 ...
 const fileInput=q('#fileInput'),browse=q('#browseBtn'),dropzone=q('#dropzone'),remove=q('#removeFile');
 ...
 const bindAux=(kind: 'pdf'|'metadata'|'av',browseSelector: string,inputSelector: string,removeSelector: string)=>{
  const auxBrowse=q(browseSelector),auxInput=q(inputSelector),auxRemove=q(removeSelector);
  if(auxBrowse&&auxInput)auxBrowse.onclick=()=>auxInput.click();
  if(auxInput)auxInput.onchange=(e: any)=>setAuxFile(kind,e.target.files[0]);
  if(auxRemove)auxRemove.onclick=()=>setAuxFile(kind,null);
 };
 bindAux('pdf','#browsePdf','#pdfInput','#removePdf');
 bindAux('metadata','#browseMetadata','#metadataInput','#removeMetadata');
 bindAux('av','#browseAv','#avInput','#removeAv');

 const institution=q('#institutionSelect'),unit=q('#unitSelect'),program=q('#programSelect'),workType=q('#workType'),workVariant=q('#workVariant'),department=q('#departmentSelect'),methodology=q('#methodologySelect');
 if(institution)institution.onchange=()=>{populateUnits();syncProfileContext();updatePackageUi()};
 if(unit)unit.onchange=()=>{populatePrograms();syncProfileContext();updatePackageUi()};
 if(program)program.onchange=()=>{autoSelectWorkType();syncProfileContext();updatePackageUi()};
 if(workType)workType.onchange=()=>{syncProfileContext();updatePackageUi()};
 if(workVariant)workVariant.onchange=()=>{syncProfileContext();updatePackageUi()};
 if(department)department.onchange=syncProfileContext;
 if(methodology)methodology.onchange=syncProfileContext;
 const citation=q('#citationStyle');if(citation)citation.onchange=()=>{updateProfile();savePreferences()};
 const language=q('#docLanguage');if(language)language.onchange=savePreferences;
 const strictness=q('#strictness');if(strictness)strictness.onchange=savePreferences;
 const submission=q('#submissionPhase');if(submission)submission.onchange=()=>{savePreferences();updateSubmissionContextFields();updatePackageUi();if(currentResult)renderSubmissionChecklist(currentResult)};
 const cohort=q('#fpzgCohort');if(cohort)cohort.onchange=()=>{populateFpzgDeadlineOptions();savePreferences();if(currentResult)renderSubmissionChecklist(currentResult)};
 const deadline=q('#fpzgDeadline');if(deadline)deadline.onchange=()=>{savePreferences();if(currentResult)renderSubmissionChecklist(currentResult)};
 const mentor=q('#mentorOverride');if(mentor)mentor.onchange=()=>{q('#customSettings')?.classList.toggle('hidden',!mentor.checked);updateProfile();savePreferences()};
 const analyze=q('#analyzeBtn');if(analyze)analyze.onclick=runAnalysis;
 const cancel=q('#cancelAnalysisBtn');if(cancel)cancel.onclick=cancelAnalysis;
 const demo=q('#demoBtn');if(demo)demo.onclick=()=>runDemo('fpzg');

 qa('.tab').forEach((tab: any)=>{tab.onclick=()=>openTab(tab.dataset.tab);tab.onkeydown=(e: any)=>{if(!['ArrowRight','ArrowLeft','Home','End'].includes(e.key))return;const tabs=Array.from<any>(doc.querySelectorAll('.tab')).filter((candidate: any)=>candidate.offsetParent!==null);const index=tabs.indexOf(tab);if(index<0)return;e.preventDefault();const next=e.key==='Home'?0:e.key==='End'?tabs.length-1:e.key==='ArrowRight'?(index+1)%tabs.length:(index-1+tabs.length)%tabs.length;const target=tabs[next];openTab(target.dataset.tab);target.focus()}});
 const detailsToggle=q('#detailsToggle'),details=q('#tabDetails');if(detailsToggle&&details)detailsToggle.onclick=()=>{const hidden=details.hasAttribute('hidden');if(hidden){details.removeAttribute('hidden');detailsToggle.classList.add('open');detailsToggle.setAttribute('aria-expanded','true');openTab('formatting')}else{details.setAttribute('hidden','');detailsToggle.classList.remove('open');detailsToggle.setAttribute('aria-expanded','false');openTab('overview')}};
 ...
 const readable=q('#previewModeCitljivo');if(readable)readable.onclick=()=>setPreviewMode('citljivo');
 const facsimile=q('#previewModeFaksimil');if(facsimile)facsimile.onclick=()=>setPreviewMode('faksimil');
 for(const selector of ['#closePreview','#closePreviewBottom']){const close=q(selector);if(close)close.onclick=closePreview}
 const preview=q('#previewModal');if(preview)preview.onclick=(e: any)=>{if(e.target.id==='previewModal')closePreview()};
 const wrong=q('#reportWrongCheck');if(wrong)wrong.onclick=reportWrongCheck;
 if(q('#closeReport'))$('#closeReport').onclick=closeReport;
 if(q('#cancelReport'))$('#cancelReport').onclick=closeReport;
 if(q('#submitReport'))$('#submitReport').onclick=submitReport;
 const report=q('#reportModal');if(report)report.onclick=(e: any)=>{if(e.target.id==='reportModal')closeReport()};
 const print=q('#printReport');if(print)print.onclick=()=>window.print();
 const downloadHtml=q('#downloadHtml');if(downloadHtml)downloadHtml.onclick=downloadHtmlReport;
 const unlock=q('#unlockReport');if(unlock){unlock.onclick=handleUnlockReport;unlock.classList.toggle('hidden',!reportEndpointConfigured())}
...
function bindCommerceControls(doc: Document){
 ...
 const cancelOrder=q('#cancelOrder');if(cancelOrder)cancelOrder.onclick=closeOrder;
 ...
 const accept=q('#analyticsAccept');if(accept)accept.onclick=()=>setAnalyticsConsent('granted');
 const decline=q('#analyticsDecline');if(decline)decline.onclick=()=>setAnalyticsConsent('denied');
 const closeAuthButton=q('#closeAuth');if(closeAuthButton)closeAuthButton.onclick=closeAuth;
 ...
 for(const selector of ['#closeCheckoutConsent','#cancelCheckoutConsent']){const close=q(selector);if(close)close.onclick=closeCheckoutConsent}
 const checkout=q('#checkoutConsentModal');if(checkout)checkout.onclick=(e: any)=>{if(e.target.id==='checkoutConsentModal')closeCheckoutConsent()};
}

function bindHistoryControls(doc: Document){
 const q=(selector: string)=>$(selector,doc);
 const history=q('#historyBtn');if(history&&q('#historyModal'))history.onclick=openHistory;
 for(const selector of ['#closeHistory','#closeHistoryBottom']){const close=q(selector);if(close)close.onclick=closeHistory}
 const clear=q('#clearHistory');if(clear)clear.onclick=clearAnalysisHistory;
 const historyModal=q('#historyModal');if(historyModal)historyModal.onclick=(e: any)=>{if(e.target.id==='historyModal')closeHistory()};
 const historyList=q('#historyList');if(historyList)historyList.onclick=handleHistoryAction;
 const repair=q('#repairHistoryBtn');if(repair&&q('#repairHistoryModal'))repair.onclick=openRepairHistory;
 for(const selector of ['#closeRepairHistory','#closeRepairHistoryBottom']){const close=q(selector);if(close)close.onclick=closeRepairHistory}
 const repairModal=q('#repairHistoryModal');if(repairModal)repairModal.onclick=(e: any)=>{if(e.target.id==='repairHistoryModal')closeRepairHistory()};
...
function bindDevControls(doc: Document){
 ...
 for(const selector of ['#closeQa','#closeQaBottom']){const close=q(selector);if(close)close.onclick=closeQa}
 const qaModal=q('#qaModal');if(qaModal)qaModal.onclick=(e: any)=>{if(e.target.id==='qaModal')closeQa()};
 const manifest=q('#downloadManifest');if(manifest)manifest.onclick=downloadProfileManifest;
 const closeSetupButton=q('#closeSetup');if(closeSetupButton)closeSetupButton.onclick=closeSetup;
 const setupModal=q('#setupModal');if(setupModal)setupModal.onclick=(e: any)=>{if(e.target.id==='setupModal')closeSetup()};
...
function bindDocumentControls(doc: Document){
 doc.addEventListener('keydown',(e: any)=>{
  if(e.key==='Escape'){
   ...
   if($('#orderModal',doc))closeOrder();
   if($('#historyModal',doc))closeHistory();
   if($('#legalModal',doc))closeLegal();
   if($('#authModal',doc))closeAuth();
   if($('#guaranteeModal',doc))closeGuarantee();
   closeCheckoutConsent();closeReport();
   if($('#previewModal',doc))closePreview();
   if($('#repairHistoryModal',doc))closeRepairHistory();
   if(__DEV_TOOLS__){if($('#qaModal',doc))closeQa();if($('#setupModal',doc))closeSetup()}
...
function revealAnalyzerForm(pick: any){if(!_engagedTracked){_engagedTracked=true;void trackEvent('analyzer_engaged',{pick:!!pick})}const col=runtimeDocument().querySelector('.lek-col-form');if(col)col.classList.add('lek-engaged');if(pick){try{$('#fileInput')?.click()}catch(e: any){}}else{try{$('#dropzone')?.focus({preventScroll:true})}catch(e: any){}}}
...
async function applyDetectedContext(file: any,admissionToken?: number){
 const token=++_detectToken,ctx=await detectDocxContext(file);
 if(token!==_detectToken||(admissionToken!==undefined&&admissionToken!==_intakeToken)||selectedDocx!==file||!ctx)return;
...
async function renderPreviewMode(){const ctx=_previewCtx;if(!ctx)return;const {r,flags,mode}=ctx;const docEl=$('#previewDoc'),side=$('#previewSide'),caption=$('#previewCaption');if(!docEl)return;docEl.textContent='Pripremam pregled...';const rendered=mode==='faksimil'?(await import('../preview/render-facsimile')).renderFacsimile(r.preview,flags):(await import('../preview/render-preview')).renderPreview(r.preview,flags);if(_previewCtx!==ctx||_previewCtx.mode!==mode)return;docEl.textContent='';docEl.classList.toggle('is-facsimile',mode==='faksimil');docEl.appendChild(rendered.root);
 ...
 $('#previewZoomBar')?.remove();
  if(mode==='faksimil'){try{const {attachFacsimileZoom,createZoomControls}=await import('../preview/facsimile-zoom');const z=attachFacsimileZoom(docEl,rendered.root);ctx.zoomApi=z;z.remeasure();if(ctx.zoomState)z.setState(ctx.zoomState);else z.fitWidth();const bar=createZoomControls(runtimeDocument(),[z]);bar.id='previewZoomBar';caption?.parentElement?.insertBefore(bar,caption)}catch(e: any){console.error('Zoom:',e)}}
...
function saveAnalysisHistory(result: any){try{persistAnalysisHistory(result)}catch(e: any){console.error('Analysis history:',e)}}
function updateHistoryBadge(){const badge=$('#historyCount');if(!badge)return;const n=getAnalysisHistory().length;badge.textContent=n?`(${n})`:''}
function openHistory(){const modal=$('#historyModal');if(!modal)return;renderHistory();modal.classList.remove('hidden');trapModal(modal)}
function closeHistory(){const modal=$('#historyModal');if(!modal)return;modal.classList.add('hidden');releaseModal(modal)}
function renderHistory(){const list=$('#historyList');if(!list)return;const h=getAnalysisHistory();const _p=activeDocumentProgress(h);const _pb=(_p&&_p.revisions>=2)?`<div class="preflight-box" style="margin-bottom:10px"><div class="preflight-head"><strong>Napredak: ${escapeHtml(_p.fileName)}</strong><span class="preflight-score">${escapeHtml(describeProgress(_p))}</span></div><div class="readiness-meter"><i style="width:${clamp(Number(_p.latestScore),0,100)}%"></i></div></div>`:'';list.innerHTML=_pb+(h.length?h.map((x: any)=>`<article class="history-card"><div class="history-head"><div><h4>${escapeHtml(x.fileName)}</h4><p>${escapeHtml(x.profile)}</p></div><div class="history-score">${Number.isFinite(Number(x.score))?Number(x.score):'?'}</div></div><div class="history-meta"><span>${escapeHtml(new Date(x.generatedAt).toLocaleString('hr-HR'))}</span><span>${Number(x.issueCount)||0} problema</span><span>${Number(x.errors)||0} važnih</span>${x.fingerprint?`<span>${escapeHtml(x.fingerprint)}</span>`:''}</div><div class="history-actions"><button class="btn btn-secondary btn-sm" data-history-profile="${escapeHtml(x.id)}">Učitaj profil</button><button class="btn btn-ghost btn-sm" data-history-delete="${escapeHtml(x.id)}">Ukloni</button></div></article>`).join(''):'<div class="empty">Još nema spremljenih lokalnih rezultata.</div>')}
...
function openRepairHistory(){const modal=$('#repairHistoryModal');if(!modal)return;modal.classList.remove('hidden');trapModal(modal);void renderRepairHistoryList()}
function closeRepairHistory(){const modal=$('#repairHistoryModal');if(!modal)return;modal.classList.add('hidden');releaseModal(modal)}
...
// <1000 rijeci I nula naslova u Word stilovima) pa je lazna uzbuna na stvarnom radu neizgledna.
 if(_gw){result.issues=result.issues||[];result.issues.unshift(issue('error','structure','Dokument ne izgleda kao završni ili diplomski rad',`${_gw} Provjeri je li učitana prava datoteka; rezultat provjere za ovakav dokument nije reprezentativan.`,'Cijeli dokument'))}}if(token!==_analyzeToken||selectedDocx!==docxFile)return;currentResult=result;analyzedProfile=p;void trackEvent('analysis_completed',{profileStatus:p.statusKey||'generic',workType:settings.workType||'',issueCount:result.issues?.length||0});withViewTransition(()=>renderResult(currentResult));if(!result.demo)emitAnalyzerResult({result,profile:p});saveAnalysisHistory(result)}catch(e: any){if(token!==_analyzeToken||isAnalysisCancelled(e))return;if(_specHit)clearSpec();console.error(e);currentResult=null;analyzedProfile=null;currentPdfAudit=null;currentMetadataAudit=null;toast(analysisErrorMessage(e));$('#progressView').classList.add('hidden');$('#wizardView').classList.remove('hidden')}finally{if(token===_analyzeToken&&analyzeBtn)analyzeBtn.disabled=false}}
...
function runDemo(kind='fpzg'){void trackEvent('demo_played',{kind});runtimeDocument().querySelector('.lek-col-form')?.classList.add('lek-engaged');currentResult=demoResult(kind);analyzedProfile=null;withViewTransition(()=>{$('#wizardView').classList.add('hidden');$('#progressView').classList.add('hidden');renderResult(currentResult)});toast((DEMO_VARIANTS[kind]||DEMO_VARIANTS.fpzg).toast);runtimeDocument().querySelector('#analyzer')?.scrollIntoView({behavior:'smooth'})}
...
function renderPhaseFourTrust(r: any){
  ...
  const trust=existing||runtimeDocument().createElement('details');
  ...
  if(m){
    ...
    if(finding){
      ...
      if(target){
        ...
        triggerBtn?.click();
        const ledgerRow: any=runtimeDocument().querySelector(`.lekta-repair-ledger-row[data-rule-id="${target.ruleId}"]`);
...
function animateBars(){const A=_animate();runtimeDocument().querySelectorAll('#categoryGrid .bar > i').forEach((el: any)=>{const p=Number(el.dataset.fill)||0;if(motionReduced()||typeof A!=='function'){el.style.width=p+'%';return}A(el,{width:['0%',p+'%']},{duration:.8,ease:[.22,1,.36,1],delay:.12})})}
function syncPremiumResultVisuals(r: any){
  ...
  Object.entries(r?.categories||{}).forEach(([key,value]: any)=>{const card=runtimeDocument().querySelector<HTMLElement>(`#resultCategoryChart [data-category="${key}"]`);if(!card)return;const percent=value?.max?Math.round(Number(value.earned||0)/Number(value.max)*100):0;const label=card.querySelector('span'),number=card.querySelector('b'),bar=card.querySelector<HTMLElement>('[data-premium-progress]');if(label)label.textContent=names[key]||key;if(number)number.textContent=`${percent}%`;if(bar){bar.dataset.value=String(percent);setProgressValue(bar,percent,100)}});
...
function reportWrongCheck(){if(!currentResult)return;const modal=$('#reportModal');if(!modal)return;const t=$('#reportNote');if(t)t.value='';modal.classList.remove('hidden');trapModal(modal)} function closeReport(){const modal=$('#reportModal');if(!modal)return;modal.classList.add('hidden');releaseModal(modal)} function submitReport(){if(!currentResult)return;const note=String($('#reportNote')?.value||'').trim();if(!note){toast('Upiši kratki opis prije slanja.');$('#reportNote')?.focus();return}const ctx: any={type:'wrong-check',appVersion:APP_VERSION,at:new Date().toISOString(),profileId:currentResult.details?.profileDefinitionId||currentResult.profile||null,profileFingerprint:currentResult.profileFingerprint||null,selection:currentResult.selection||null,file:currentResult.file?.name||null,score:currentResult.score??null,note:note.slice(0,1000)};closeReport();const contact=productionConfig.contactEmail;if(contact&&!/nije konfiguriran/i.test(contact)){location.href=`mailto:${contact}?subject=${encodeURIComponent('Lekta: prijava pogrešne provjere')}&body=${encodeURIComponent('Prijava pogrešne provjere\n\n'+JSON.stringify(ctx,null,2))}`}else{downloadBlob(JSON.stringify(ctx,null,2),'application/json',`Lekta-prijava-provjere-${ctx.profileFingerprint||'rad'}.json`);toast('Kontakt nije konfiguriran; preuzet je JSON prijave za ručnu predaju.')}trackEvent('wrong_check_reported',{profileId:ctx.profileId||''})}
...
function ensureResultScrollProgress(){const doc=runtimeDocument();if(doc.getElementById('lekResultProgress'))return;try{const bar=doc.createElement('div');bar.id='lekResultProgress';bar.setAttribute('aria-hidden','true');doc.body.appendChild(bar)}catch(e: any){}}
...
function downloadBlob(content: any,type: any,name: any){const blob=new Blob([content],{type}),a=runtimeDocument().createElement('a');a.href=URL.createObjectURL(blob);a.download=name;a.click();setTimeout(()=>URL.revokeObjectURL(a.href),1000)}
...
function renderTextItemsSection(host: any,r: any,textItems: any[]){
 ...
 const box=runtimeDocument().createElement('div');box.className='lekta-repair-text';
 ...
 for(const it of textItems){
  const row=runtimeDocument().createElement('div');row.className='lekta-repair-text__row';row.dataset.ruleId=it.ruleId;
...
function renderRepairCapabilityBlock(mount: any,r: any): boolean{
 ...
 const wrap=runtimeDocument().createElement('div');wrap.className='lekta-repair-panel';
 const blocked=runtimeDocument().createElement('p');blocked.className='lekta-repair-panel__blocked';
...
function renderServerRepairPanel(mount: any,r: any,items: any[],file: any,textItems: any[]=[]){
 const wrap=runtimeDocument().createElement('div');wrap.className='lekta-repair-panel';
 const intro=runtimeDocument().createElement('p');
 ...
 const list=runtimeDocument().createElement('ul');list.className='lekta-repair-panel__list';
 ...
 ordered.forEach((item: any,orderIdx: number)=>{
  ...
  if(orderIdx===firstExtraIdx&&firstExtraIdx!==-1){
   const sub=runtimeDocument().createElement('li');sub.className='lekta-repair-panel__subtitle';
  ...
  const li=runtimeDocument().createElement('li');li.className='lekta-repair-panel__item';li.dataset.ruleId=item.ruleId;
 ...
 if(deepAvailable){
  const deepRow=runtimeDocument().createElement('label');deepRow.className='lekta-repair-panel__deep';
 ...
 const consentRow=runtimeDocument().createElement('label');consentRow.className='lekta-repair-panel__deep';
 ...
 const consentHint=runtimeDocument().createElement('p');consentHint.className='lekta-repair-panel__consent-hint';consentHint.hidden=true;consentHint.textContent='Prvo označi privolu iznad, pa klikni ponovno.';
 wrap.appendChild(consentHint);
 const btn=runtimeDocument().createElement('button');btn.type='button';btn.className='lekta-repair-panel__download';btn.textContent='Popravi sve jednim klikom';
 ...
 const confirmBox=runtimeDocument().createElement('div');confirmBox.className='lekta-repair-panel__confirm-box';confirmBox.hidden=true;
 wrap.appendChild(confirmBox);
 const summary=runtimeDocument().createElement('div');summary.className='lekta-repair-panel__summary';summary.hidden=true;
 ...
 async function go(confirmedMismatch: boolean){
  ...
  try{
   ...
   if(out.kind==='ok'&&out.changelog.length===0){
    ...
    const addHtml=(html: string)=>{const d=runtimeDocument().createElement('div');d.innerHTML=html;summary.appendChild(d);return d};
    ...
    try{const f=new File([out.docxBytes as Uint8Array<ArrayBuffer>],out.fileName,{type:DOCX_MIME});const res: any=await analyzeDocxOffThread(f,analyzedProfile,r.settings,()=>{},{skipFinalDelay:true});
     ...
     if(res?.preview&&r.preview){
      const b=runtimeDocument().createElement('button');b.type='button';b.className='btn btn-secondary btn-sm';b.textContent='Pokaži što je popravljeno';
    ...
    const reloadBtn=runtimeDocument().createElement('button');
