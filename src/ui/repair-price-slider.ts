...
export function renderRepairLedgerModal<T extends PriceSliderItem>(opts: PriceSliderOptions<T>): HTMLElement {
  ...
  function openItemModal(descriptor: AdvancedFormDescriptor, onBack: () => void): void {
    ...
    // ISTI RAZLOG KAO U `openLedger` NIZE: Escape ide na `document`, ne na `itemBackdrop`, jer
    // WebKit gumbe ne fokusira na klik misem pa fokus zna ostati izvan modala. Ovaj backdrop je
    // PRIVREMEN (gradi se i brise po otvaranju), pa se slusalac uklanja u `closeItem`.
  ...
  // ESCAPE IDE NA `document`, NE NA `backdrop` (korak D, 2026-09-13). Klik na redak unutar ledgera
  // toggla kucicu preko `cb.dispatchEvent(new Event('change'))`, a WebKit gumbe ne fokusira na klik
  // misem (poznato ponasanje, bez "Full Keyboard Access"), pa `document.activeElement` zna ostati
  // na `<body>` cak i dok je ledger otvoren. `backdrop`-ov vlastiti `keydown` bi tad NIKAD ne primio
  // Escape, jer se dogadjaji penju od fokusiranog elementa PREMA GORE kroz pretke, a `<body>` je
  // predak od `backdrop`, ne potomak. IZMJERENO 2026-09-13 (`tests/ux/repair-cta-opens-panel.spec.ts`,
  // webkit): nakon klika na redak `document.activeElement` je bio `<body>` (`inside: false`), i
  // Escape zato nije zatvorio ledger ni nakon 200 ms. `document`-ova slusalac ne ovisi o tome gdje je
  // fokus; zastavica `hidden` sprjecava zatvaranje kad ledger vec nije otvoren.
  // NALAZ D2: ovaj slusac se do sada vjesao na `document` bez ikad uklanjanja (za razliku od
  // item-modala iznad, koji svoj `onEscape` skida u `closeItem`), pa se pri svakom ponovnom
  // pozivu `renderRepairLedgerModal` (novi panel = nov `backdrop`, isti `document`) gomilao jos
  // jedan slusac koji trajno zivi na `document`. Imenovan je i uklanja se u `closeLedger`, dakle
  // pri SVAKOM putu zatvaranja (gumb Gotovo, X, klik na pozadinu, Escape).
  function onLedgerEscape(e: KeyboardEvent): void {
    if (e.key === 'Escape' && !backdrop.classList.contains('hidden')) closeLedger();
  }
  function openLedger(): void {
    renderAll(); // odabir je mogao promijeniti kontroler (plan, T09) dok je modal bio zatvoren
    backdrop.classList.remove('hidden');
    trapModal(backdrop);
    document.addEventListener('keydown', onLedgerEscape);
  }
  ...
  function closeLedger(): void {
    ...
    document.removeEventListener('keydown', onLedgerEscape);
  }
