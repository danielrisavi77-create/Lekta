/**
 * Modal placanja (F18, 2026-09-23): Stripe Payment Element u stranici.
 *
 ...
 *
 * Ovisnosti o `app.ts` se PRIMAJU kao argumenti (toast, trapModal, releaseModal, trackEvent,
 * onPaid), a ne uvoze, da modul ne zatvori kruzni uvoz s monolitom koji ga lijeno ucitava.
 *
 ...
 * `onPaid` je `handleUnlockReport`, koji provjeri pravo i otkljuca izvjestaj.
 */
...
export interface StripePaymentArgs {
  out: { clientSecret: string; publishableKey: string };
  productId: string;
  ...
  pay.onclick = async () => {
    ...
    try {
      ...
      if (res.kind === 'ok') {
        ...
        args.toast('Plaćanje je uspjelo. Otključavam izvještaj…');
        args.trackEvent('purchase_completed', { product: args.productId });
        await args.onPaid();
        return;
      }
