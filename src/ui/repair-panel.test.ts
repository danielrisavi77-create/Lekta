...
function chk(title: string, status: string, earned: number, max: number): Check {
  ...
  return { id: checkIdFor('formatting', title), evidence: max > 0 ? 'measured' : 'not-applicable', measurementStatus: max > 0 ? 'measured' : 'not-applicable', category: 'formatting', title, status, earned, max, detail: '', issue: null, scored: max > 0 };
}
