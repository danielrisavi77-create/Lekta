...
function validateRequiredSections(params: DataObject): boolean {
  ...
  return array(params.sections, 50, (item) => object(item, ['id', 'kind', 'label', 'insertionAnchor', 'headingLevel', 'numbered', 'confirmed'], ['styleId', 'placeholderText', 'commentText', 'statementText'])
    ...
    && object(item.insertionAnchor, ['paragraphIndex', 'anchorFingerprint', 'position'], ['anchorText'])
    && integer(item.insertionAnchor.paragraphIndex) && fingerprint(item.insertionAnchor.anchorFingerprint)
    && optional(item.insertionAnchor.anchorText, (value) => text(value))
...
function validateLinkDoi(params: DataObject): boolean {
  ...
  return array(params.operations, 1_000, (item) => object(item, ['id', 'part', 'paragraphIndex', 'start', 'end', 'anchorFingerprint', 'before', 'action', 'confirmed'], ['anchorText', 'replacementText', 'targetUrl'])
    ...
    && fingerprint(item.anchorFingerprint) && optional(item.anchorText, (value) => text(value))
    && text(item.before, 4_000) && optional(item.replacementText, (value) => text(value, 4_000))
