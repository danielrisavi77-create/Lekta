...
-- Stores structured workflow metadata only: never raw thesis/document text,
-- mentor-message bodies, prompts, model outputs, source passages, or generic
-- JSON payload columns.

create or replace function public.set_completion_updated_at()
returns trigger
language plpgsql
...
revoke all on function public.set_completion_updated_at() from public, anon, authenticated;
grant execute on function public.set_completion_updated_at() to service_role;
...
create table if not exists public.completion_tasks (
  ...
  constraint completion_tasks_id_project_key unique (id, academic_project_id)
);
...
create table if not exists public.completion_events (
  ...
  constraint completion_events_task_project_fkey
