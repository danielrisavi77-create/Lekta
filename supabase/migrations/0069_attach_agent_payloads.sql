-- Bind pre-run temporary materials to the canonical run after the run exists.
-- Raw material content remains private storage; this function moves metadata only.
  ...
  where m.user_id = p_user_id
    ...
    and (m.run_id is null or not exists (
      ...
      where previous.run_id = m.run_id
        and previous.status in ('pending', 'running', 'paused')
    ))
