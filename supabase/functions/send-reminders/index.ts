...
async function processSlotExpiryReminders(): Promise<{ sent: number }> {
  ...
  for (const slot of slots) {
    // Preskoci ako je korisnik ugasio ovu vrstu podsjetnika (default: ukljuceno).
    ...
    if (pref && pref.slot_expiry_reminders_enabled === false) continue;

    // Preskoci ako je bilo generacije u zadnja 24h (korisnik je vec aktivan na tom radu).
